...
describe('WarehouseEvidenceCard', () => {
  ...
  it('renders conservative, source-labelled report-only findings when supplied', () => {
    ...
    expect(screen.getByTestId('warehouse-evidence-caution').textContent).toContain('Coverage remains bounded staging only');
  ...
  it('renders live canonical and observed findings without claiming warehouse-only scope', () => {
    ...
    expect(screen.getByTestId('warehouse-evidence-planner-boundary').textContent).toContain('Planner truth remains canonical');
  });
  ...
  it('renders not-evaluated freshness without implying fresh evidence', () => {
    ...
    expect(screen.getByTestId('warehouse-evidence-summary').textContent).toContain(
      'Evidence was not safely evaluated in this runtime.',
    );
    openTechnicalDetail();
  ...
  it('renders stale, risky/needs-review, unresolved, and blocked findings conservatively', () => {
    ...
    expect(screen.getByTestId('warehouse-evidence-caution').textContent).toContain('Manual review remains required');
  ...
  it('keeps the disclosure keyboard-focusable while exposing no mutation controls', () => {
    const evidence: PlannerWarehouseEvidence = {
      availability: 'report_only',
      reportOnly: true,
      items: [
    ...
    const toggle = screen.getByTestId('warehouse-evidence-disclosure-toggle');
    toggle.focus();
    expect(document.activeElement).toBe(toggle);
    expect(toggle.getAttribute('aria-expanded')).toBe('false');
    fireEvent.click(toggle);
    expect(toggle.getAttribute('aria-expanded')).toBe('true');
    expect(screen.getByTestId('warehouse-evidence-disclosure-panel')).toBeTruthy();

    expect(container.querySelectorAll('button').length).toBe(1);
    expect(container.querySelectorAll('a').length).toBe(0);
    expect(container.querySelectorAll('input, select, textarea').length).toBe(0);
    expect(screen.queryByRole('button', { name: /save|apply|promote|archive|retry/i })).toBeNull();
  });
});
