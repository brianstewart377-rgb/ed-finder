...
export function PlannerRouteGuard() {
  ...
  if (!mismatch && !direct && !unavailable) {
    return <RouteSurface title="Loading selected system" detail="Resolving the requested Planner system without selecting or creating a local draft." systemId={id64} busy />;
  }
  ...
  if (mismatch) {
    return <RouteSurface title="Requested draft could not be opened" detail="This draft is missing, archived, or belongs to a different system. A different local draft has not been opened instead." systemId={id64} primary="View system without a draft" onPrimary={() => { window.location.hash = `#colony-planner/system/${id64}`; }} testId="planner-project-route-error" />;
  }
  if (unavailable) {
    return <RouteSurface title="Requested system could not be opened" detail={error ?? 'The selected system is unavailable.'} systemId={id64} testId="planner-system-route-error" />;
  }
  return <RouteSurface title="No active draft for this system" detail="This direct Planner route has not chosen or created a local draft." systemName={system?.name || 'Unknown system'} systemId={id64} primary="Create draft" onPrimary={createDraft} create testId="planner-no-active-draft-route" />;
}

function RouteSurface({ title, detail, systemName, systemId, primary, onPrimary, create, busy, testId }: { title: string; detail: string; systemName?: string; systemId: number; primary?: string; onPrimary?: () => void; create?: boolean; busy?: boolean; testId?: string }) {
  return (
    <section role="dialog" aria-modal="true" aria-label={title} data-testid={testId ?? 'planner-route-loading'} className="fixed inset-0 z-[60] grid place-items-center bg-bg0/95 p-4 backdrop-blur-sm">
      <div className="panel w-full max-w-2xl border-orange/35 p-6 sm:p-8">
        ...
        {systemName ? (
          <div data-testid="planner-selected-system-context" className="mt-3 rounded border border-cyan/30 bg-bg3/50 p-3">
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-silver-dk">Selected system</p>
            <p className="mt-1 text-base font-semibold text-text">{systemName}</p>
            <p className="mt-2 font-mono text-[10px] uppercase tracking-[0.14em] text-silver-dk">System detail available · ID64 {systemId}</p>
          </div>
        ) : null}
        <h1 className="mt-4 font-display text-xl tracking-[0.12em] text-orange-lt">{title}</h1>
