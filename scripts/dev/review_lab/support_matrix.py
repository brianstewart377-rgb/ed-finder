...
REVIEW_SUPPORT_ROUTE_MATRIX: tuple[SupportRoute, ...] = (
    ...
    SupportRoute(
        ...
        frontend_caller='EliteNewsBar background query',
        required_for_reviewed_flow=True,
        expected_status=200,
        review_only_handling='Review-only empty synthetic news payload; no external fetch or source claim.',
        allowed_response_characteristics=('JSON object', 'items array empty', 'fixed review-only timestamp', 'no external fetch'),
        scenario_coverage=('planner_core', 'empty_optional_support_data', 'support_route_compatibility'),
        validation_mode='api_contract_validated',
    ),
    SupportRoute(
    ...
    SupportRoute(
        route='/api/archetypes/system/{id64}',
        frontend_caller='System Detail archetype assessment',
        required_for_reviewed_flow=True,
        expected_status=200,
        review_only_handling='Normal read-only archetype handler against the review database.',
        allowed_response_characteristics=('JSON object', 'id64/name/archetypes keys', 'real route shape'),
        scenario_coverage=('planner_core', 'evidence_available', 'evidence_unavailable', 'evidence_unknown', 'evidence_not_evaluated', 'partial_optional_data', 'support_route_compatibility'),
        validation_mode='api_contract_validated',
    ),
    SupportRoute(
        route='/api/evidence/systems/{id64}/summary',
        frontend_caller='System Detail evidence posture summary',
        required_for_reviewed_flow=True,
        expected_status=200,
        review_only_handling='Normal read-only evidence summary handler against the review database.',
        allowed_response_characteristics=('JSON object', 'system_id64/count/list keys', 'real route shape'),
        scenario_coverage=('planner_core', 'evidence_available', 'evidence_unavailable', 'evidence_unknown', 'evidence_not_evaluated', 'partial_optional_data', 'support_route_compatibility'),
        validation_mode='api_contract_validated',
    ),
    SupportRoute(
