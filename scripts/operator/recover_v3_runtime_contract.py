...
SENSITIVE_ASSIGNMENT_NAME = (
    r"(?:PASSWORD|PASSWD|SECRET|TOKEN|API_KEY|PRIVATE_KEY|SECRET_ACCESS_KEY|"
    r"[A-Za-z_][A-Za-z0-9_]*_(?:PASSWORD|PASSWD|SECRET|TOKEN|API_KEY|PRIVATE_KEY|SECRET_ACCESS_KEY))"
)
URI_RE = re.compile(
    r"(?i)\b(postgresql|postgres|redis|rediss)://([^\s/@:]*):([^\s/@]+)@"
)
URI_PASSWORD_QUERY_RE = re.compile(
    r"(?i)(\b(?:postgresql|postgres|redis|rediss)://[^\s#'\"]*?[?&]password=)([^&#\s'\"]+)"
)
QUOTED_ASSIGNMENT_RE = re.compile(
    rf"(?im)^(?P<prefix>\s*(?:-\s*)?(?:export\s+)?{SENSITIVE_ASSIGNMENT_NAME}\s*=\s*)(?P<quote>['\"])(?P<value>[^\r\n'\"]*)(?P=quote)(?P<tail>\s*(?:#.*)?)$"
)
SHELL_ASSIGNMENT_RE = re.compile(
    rf"(?im)^(?P<prefix>\s*(?:export\s+)?{SENSITIVE_ASSIGNMENT_NAME}\s*=\s*)(?P<value>[A-Za-z0-9_./:@%+,~-]+)(?P<tail>\s*(?:#.*)?)$"
)
YAML_ASSIGNMENT_RE = re.compile(
    rf"(?im)^(?P<prefix>\s*(?:-\s*)?{SENSITIVE_ASSIGNMENT_NAME}\s*:\s*)(?P<value>[A-Za-z0-9_./:@%+,~-]+)(?P<tail>\s*(?:#.*)?)$"
)
DOCKER_ASSIGNMENT_RE = re.compile(
    rf"(?im)^(?P<prefix>\s*(?:ENV|ARG)\s+{SENSITIVE_ASSIGNMENT_NAME}\s*=\s*)(?P<value>[^\s#]+)(?P<tail>\s*(?:#.*)?)$"
)
...
def _is_template(value: str) -> bool:
    ...
    return (
        ...
        or re.fullmatch(r"(?i)REPLACE(?:_|-)ME(?:_|-)WITH(?:_|-)[A-Z0-9_-]+", value)
        is not None
...
def _redact_uri(match: re.Match[str]) -> str:
    if _is_template(match.group(3)):
        return match.group(0)
    return f"{match.group(1)}://{REDACTION_SENTINEL}:{REDACTION_SENTINEL}@"


def _redact_query_password(match: re.Match[str]) -> str:
    if _is_template(match.group(2)):
        return match.group(0)
    return f"{match.group(1)}{REDACTION_SENTINEL}"


def _assignment_patterns(relative: str | None) -> tuple[re.Pattern[str], ...]:
    if relative is None:
        return (QUOTED_ASSIGNMENT_RE,)
    name = PurePosixPath(relative).name.lower()
    suffix = PurePosixPath(relative).suffix.lower()
    if name.startswith(BUILD_FILE_PREFIXES):
        return (QUOTED_ASSIGNMENT_RE, DOCKER_ASSIGNMENT_RE)
    if suffix in {".yml", ".yaml"}:
        return (QUOTED_ASSIGNMENT_RE, YAML_ASSIGNMENT_RE)
    if suffix == ".sh":
        return (QUOTED_ASSIGNMENT_RE, SHELL_ASSIGNMENT_RE)
    return (QUOTED_ASSIGNMENT_RE,)


def _sanitize_text(text: str, relative: str | None = None) -> tuple[str, bool]:
    updated = URI_RE.sub(_redact_uri, text)
    updated = URI_PASSWORD_QUERY_RE.sub(_redact_query_password, updated)

    def redact_assignment(match: re.Match[str]) -> str:
        if _is_template(match.group("value")):
            return match.group(0)
        quote = match.groupdict().get("quote", "")
        return f"{match.group('prefix')}{quote}{REDACTION_SENTINEL}{quote}{match.group('tail')}"

    for pattern in _assignment_patterns(relative):
        updated = pattern.sub(redact_assignment, updated)
...
def _unsafe(text: str, relative: str | None = None) -> bool:
    if _excluded_reason(text):
        return True
    if any(not _is_template(match.group(3)) for match in URI_RE.finditer(text)):
        return True
    if any(
        not _is_template(match.group(2))
        for match in URI_PASSWORD_QUERY_RE.finditer(text)
    ):
        return True
    return any(
        not _is_template(match.group("value"))
        for pattern in _assignment_patterns(relative)
...
def preflight_files(
    ...
    for source in files:
        ...
        if b"\x00" in original:
            raise RecoveryError(
                f"Selected text file contains disallowed binary content: {source.relative}"
            )
        try:
        ...
        if source.path.suffix.lower() == ".json":
            ...
            safe_text, changed = _sanitize_text(text, source.relative)
        if _unsafe(safe_text, source.relative):
            raise RecoveryError(
...
def stream_archive(
    ...
    for item in prepared:
        if item.payload is not None and _unsafe(
            item.payload.decode("utf-8"), item.source.relative
        ):
