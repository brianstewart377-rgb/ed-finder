...
for svc in "${RUNNERS[@]}"; do
  systemctl is-active --quiet "$svc" || fail "required runner inactive: $svc"
done
...
SYSTEM_COUNT="$(env PGDATABASE="$DATABASE_URL" psql -X -Atqc 'select count(*) from systems')"
[ "$SYSTEM_COUNT" -ge 40 ] || fail "checkpoint preview dataset is incomplete"
...
python3 - "$DB_AUTHORITY" "$OBSERVED_DB" "$OBSERVED_ADDRESS" "$OBSERVED_PORT" \
  "$LEDGER_FILE" "$SCHEMA_RECEIPT" <<'PY'
import datetime, hashlib, json, pathlib, sys
source, dbname, address, port, ledger_path, out = sys.argv[1:]
root = pathlib.Path(".")
entries = []
...
for svc in "${RUNNERS[@]}"; do
  systemctl is-active --quiet "$svc" || fail "runner changed during provisioning: $svc"
done
...
runtime["tcp_listeners"] = sorted(
    line.split()[3] for line in subprocess.check_output(["ss", "-lntH"], text=True).splitlines()
)
