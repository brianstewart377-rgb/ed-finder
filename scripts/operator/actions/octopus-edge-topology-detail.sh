...
def exact_version(body):
    ...
    return any(isinstance(item, str) and re.fullmatch(r"v?" + re.escape(EXPECTED_VERSION), item)
...
def decode_inspect(stdout, expected_name):
    ...
    if raw["name"] != "/" + expected_name or raw["state"] != "running":
        raise ValueError("unexpected container identity or state")
    return {
        "name": expected_name,
        "image": raw["image"],
...
for name in CONTAINERS:
    inspected = run(["docker", "inspect", "--type", "container", "--format", INSPECT_FORMAT, name])
    if inspected.returncode != 0:
        failures.append(f"container_missing:{name}")
        continue
    try:
        item = decode_inspect(inspected.stdout, name)
    ...
    container_receipts.append(item)
...
version["exact_version_match"] = exact_version(version_body)
