...
REQUIRED_MATERIALIZED_VIEWS = {
    "mv_map_regions": "ux_mv_map_regions_cell",
    "mv_map_heatmap_200ly": "ux_mv_map_heatmap_200",
    "mv_map_heatmap_500ly": "ux_mv_map_heatmap_500",
    "mv_map_heatmap_1000ly": "ux_mv_map_heatmap_1000",
    "mv_map_timeline_month": "ux_mv_map_timeline",
    "mv_archetype_rankings": "idx_archetype_rankings_pk",
}
...
def database_target_identity(cur) -> dict[str, object]:
    cur.execute(
        "SELECT current_database(), current_user, "
        "inet_server_addr()::text, inet_server_port()"
    )
    database, user, server_address, server_port = cur.fetchone()
    return {
        "database": str(database),
        "user": str(user),
        "server_address": None if server_address is None else str(server_address),
        "server_port": None if server_port is None else int(server_port),
        "address_source": "local_socket" if server_address is None else "server_reported",
    }
...
def materialized_view_metric(cur, name: str, required_index: str) -> dict[str, object]:
    if not relation_exists(cur, name):
        return {
            "presence": "missing", "populated": None,
            "required_unique_index": required_index, "unique_index_valid": False,
        }
    populated = bool(scalar(
        cur,
        "SELECT relispopulated FROM pg_class WHERE oid=to_regclass(%s)",
        (f"public.{name}",),
    ))
    index_valid = bool(scalar(
        cur,
        """SELECT EXISTS (
               SELECT 1
                 FROM pg_index i
    ...
    return {
        "presence": "present", "populated": populated,
        "required_unique_index": required_index, "unique_index_valid": index_valid,
    }
...
def collect(cur, full: bool) -> dict[str, object]:
    ...
    views = {
        name: materialized_view_metric(cur, name, index)
        for name, index in REQUIRED_MATERIALIZED_VIEWS.items()
    }
...
def determine_readiness(report: dict[str, object]) -> tuple[str, list[str]]:
    ...
    if report["profile"] == "production_safe":
        ...
        if derived["ratings_v3_4"].get("value") != 0:
            reasons.append("ratings v3.4 coverage was not proven exactly zero")
        for dataset, metric in (("topology", "missing_for_ratings"), ("archetypes", "missing_scores_for_ratings"), ("archetypes", "missing_traits_for_scores"), ("regional_analysis", "missing_for_positioned_systems"), ("station_body_links", "required_backfill")):
            ...
            if state is None:
                ...
                reasons.append(f"{dataset} {metric} is not exactly zero")
    view_states = derived["materialized_views"].get("objects", {})
    for name, required_index in REQUIRED_MATERIALIZED_VIEWS.items():
        state = view_states.get(name, {})
        if state.get("presence") != "present" or not state.get("populated"):
            reasons.append(f"materialized view {name} is missing or unpopulated")
        if not state.get("unique_index_valid"):
            reasons.append(
                f"materialized view {name} lacks required valid unique index "
                f"{required_index}"
            )
...
def run_audit(args: argparse.Namespace) -> dict[str, object]:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            reported_target = database_target_identity(cur)
            ...
            report: dict[str, object] = {
                ...
                "profile": "full" if args.full else "production_safe",
                "target": {
                    "connection_hint": redacted_target(args.database_url),
                    "database_reported": reported_target,
...
def print_human(report: dict[str, object], receipt: Path) -> None:
    ...
    address = target["server_address"] or "local socket (address unavailable)"
    port = "unknown" if target["server_port"] is None else target["server_port"]
    print(
        f"  DB identity  : database={target['database']}, user={target['user']}, "
        f"server={address}, port={port}"
    )
    print("  CONFIRM      : verify this DB identity before any write phase")
