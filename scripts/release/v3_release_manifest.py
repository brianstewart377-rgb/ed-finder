...
def validate_manifest(
    ...
    *,
    ...
    if not isinstance(git_sha, str) or not GIT_SHA.fullmatch(git_sha):
        raise ManifestError("git_sha must be exactly 40 lowercase hexadecimal characters")
    ...
    for index, entry in enumerate(migrations["entries"]):
        item = _exact_keys(entry, {"path", "mode", "sha256"}, f"migration_set.entries[{index}]")
    ...
    if not isinstance(compatible_sets, list) or any(
        ...
        raise ManifestError("compatible_migration_sets must contain only sha256 identities")
    ...
    if evidence is not None and (not isinstance(evidence, str) or not evidence.strip()):
        raise ManifestError("schema compatibility evidence must be null or non-empty text")
    if isinstance(evidence, str) and (
        SENSITIVE_TEXT.search(evidence) or URI_USERINFO.search(evidence)
    ):
        raise ManifestError("schema compatibility evidence contains secret-like material")

    proved = status in {"exact", "backward-compatible"}
    if proved:
        if not evidence or not compatible_sets:
            raise ManifestError("proved compatibility requires evidence and migration identities")
        if migrations["identity"] not in compatible_sets:
            raise ManifestError("release migration identity must be explicitly compatible")
    elif compatible_sets or evidence is not None:
        raise ManifestError("unknown/incompatible compatibility cannot carry positive evidence")

    rollback = _exact_keys(document["rollback"], {"application_only_eligible", "reason"}, "rollback")
    ...
    if SENSITIVE_TEXT.search(rollback["reason"]) or URI_USERINFO.search(rollback["reason"]):
        raise ManifestError("rollback.reason contains secret-like material")
    if eligible and not proved:
        raise ManifestError("application-only rollback requires proved schema compatibility")
    ...
    if current_migration_set is not None:
        ...
        if not proved or current_migration_set not in compatible_sets:
            raise ManifestError("release compatibility with the current database is absent or unknown")
...
def _load(path: Path) -> object:
    try:
        ...
        raise ManifestError(f"unable to read release manifest: {type(exc).__name__}") from exc
...
def _parser() -> argparse.ArgumentParser:
    ...
    create.add_argument("--compatibility", choices=sorted(COMPATIBILITY_STATUSES), default="unknown")
    ...
    verify.add_argument(
        "--purpose",
        choices=("release", "deploy-candidate", "deploy", "rollback-candidate", "rollback"),
        default="release",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "create":
            ...
            print(json.dumps({"status": "created", "release_id": document["release_id"]}))
        else:
            ...
            if args.expected_git_sha is not None and document["git_sha"] != args.expected_git_sha:
                raise ManifestError("manifest git_sha does not match the selected release")
            print(json.dumps({"status": "verified", "release_id": document["release_id"]}))
