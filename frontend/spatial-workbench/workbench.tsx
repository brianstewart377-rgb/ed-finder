...
import { createSpatialFixture, FIXTURE_TIERS, type FixtureTier } from '../src/features/spatial-runtime/fixtures';
...
export function SpatialWorkbench() {
  ...
  useEffect(() => {
    ...
    void runtime.initialize(canvas, { preferWebGpu: true, reducedMotion, onTelemetry: setTelemetry }).then((selected) => {
      if (!active) return; setBackend(selected); runtime.loadScene(createSpatialFixture(tier)); runtime.resize(canvas.clientWidth, canvas.clientHeight, devicePixelRatio);
    }).catch(() => setBackend('initializing'));
    const observer = new ResizeObserver(([entry]) => { if (entry) runtime.resize(entry.contentRect.width, entry.contentRect.height, devicePixelRatio); });
    observer.observe(canvas);
    return () => { active = false; observer.disconnect(); runtime.dispose(); runtimeRef.current = null; };
  }, [reducedMotion]);

  const load = (next: FixtureTier) => { setTier(next); runtimeRef.current?.loadScene(createSpatialFixture(next)); };
  ...
  return <main>
    ...
    <section className="controls" aria-label="Workbench controls">
      ...
      <button onClick={() => runtimeRef.current?.setCamera({ ...runtimeRef.current.snapshot().camera, mode: 'top-down', pitchRadians: Math.PI / 2 })}>Top-down</button>
      <button onClick={() => runtimeRef.current?.setCamera({ ...runtimeRef.current.snapshot().camera, mode: 'pitched', pitchRadians: .75 })}>Restrained pitch</button>
      <button onClick={() => void runtimeRef.current?.flyTo('system:fixture-0')}>Fly to selected</button>
      ...
      <button onClick={() => void runtimeRef.current?.rebuild('manual')}>Rebuild retained CPU state</button>
