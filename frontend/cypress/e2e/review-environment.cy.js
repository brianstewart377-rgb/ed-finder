...
Cypress.on('window:before:load', (win) => {
  ...
  // Chrome reports failed subresource loads in DevTools without calling the
  // application's console methods. Capture the corresponding browser events
  // so the Python console policy cannot incorrectly accept a broken page.
  win.addEventListener('error', (event) => {
    ...
    if (target && target !== win) {
      ...
      summary.consoleEntries.push({
        type: 'error',
        text: sanitizeText(`Browser resource load failed: ${resource}`),
      });
    }
  }, true);
  win.addEventListener('securitypolicyviolation', (event) => {
    summary.consoleEntries.push({
      type: 'error',
      text: sanitizeText(`Browser security policy violation: ${event.violatedDirective || 'unknown directive'} ${event.blockedURI || ''}`),
    });
  });
