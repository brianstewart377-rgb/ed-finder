...
function assertText(testId, pattern, checks, key) {
  cy.getByTestId(testId).should('contain.text', pattern instanceof RegExp ? undefined : pattern);
...
function ensureTelemetryToggleKeyboardWorks(checks) {
  cy.get('[data-testid="planner-telemetry-dock-toggle"]:visible')
    .first()
    .should('be.visible')
    .focus()
    .should('have.focus')
...
Cypress.on('uncaught:exception', (error) => {
  ...
  // Match Playwright's pageerror listener: record the error and let the Python
  // Review Lab policy decide whether it is an allowed or blocking observation.
  return false;
});
...
describe('Local review environment verification — Cypress', () => {
  ...
  it('validates minimum laptop planner posture', () => {
    ...
    assertVisible('planner-evidence-discoverability-surface', checks, 'reportOnlyBoundaryVisible');
    assertText('planner-evidence-discoverability-summary', /canonical planner truth/i, checks, 'canonicalBoundaryVisible');
    cy.get('[data-testid="planner-telemetry-dock-toggle"]:visible').first().should('be.visible');
