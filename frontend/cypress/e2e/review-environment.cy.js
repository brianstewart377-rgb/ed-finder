...
function gotoFinder() {
  ...
  cy.contains('label', 'Colony status').should('be.visible').invoke('attr', 'for').then((controlId) => {
    ...
    cy.get(`[id="${controlId}"]`).click();
  });
  cy.contains('[role="option"]', 'Any').should('be.visible').click();
...
function activateControl(testId, keyboard) {
  const control = cy.getByTestId(testId).should('be.visible');
  if (keyboard) {
    control.focus().should('have.focus').type('{enter}');
  } else {
    control.click();
  }
}
  ...
  cy.get('body').then(($body) => {
    ...
    if (details.length > 0) {
      if (!details.prop('open')) {
        cy.getByTestId('warehouse-evidence-technical-details').find('summary').click();
      }
      // React may replace the details element when its disclosure state changes.
      // Re-query it just as the Playwright locator contract does.
...
function visibleTelemetryToggle() {
  return cy.getByTestId('planner-telemetry-dock-toggle')
    .filter(':visible')
    .first()
    .should('be.visible');
}
...
function ensureTelemetryToggleKeyboardWorks(checks) {
  visibleTelemetryToggle()
    .focus()
    .should('have.focus')
    .type('{enter}');
  // Each activation rerenders the dock. Never chain through the detached
  // pre-activation subject; resolve the visible control again for each check.
  visibleTelemetryToggle().should('have.attr', 'aria-expanded', 'true');
  visibleTelemetryToggle().focus().should('have.focus').type('{enter}');
  visibleTelemetryToggle().should('have.attr', 'aria-expanded', 'false');
  cy.then(() => {
...
function returnToFinder(checks) {
  // At phone width the sticky telemetry region can cover the pointer hitbox.
  // Keyboard activation verifies the same accessible exit without bypassing
  // Cypress actionability checks.
  cy.contains('button:visible', /Back to Finder/i)
    .should('be.visible')
    .focus()
    .should('have.focus')
    .type('{enter}');
...
describe('Local review environment verification — Cypress', () => {
  ...
  it('validates phone-width planner resilience and safe exit', () => {
    ...
    cy.contains('button:visible', /Back to Finder/i).should('be.visible').then(() => {
