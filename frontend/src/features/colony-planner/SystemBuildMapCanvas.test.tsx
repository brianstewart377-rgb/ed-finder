...
const system = {
  ...
  overall_development_potential: 72,
  archetype_score: 72,
...
describe('SystemBuildMapCanvas real data adapter', () => {
  it('renders user-facing build map copy and concise slot disclaimer', () => {
    render(<SystemBuildMapCanvas system={system} snapshot={snapshot} selection={{ type: 'system' }} onSelect={vi.fn()} />);
  ...
  it('uses calculated station baseline percentages for contextual ports when body mechanics are available', () => {
    ...
    const row = buildPlannerCanvasRows(hmcGeoSystem, contextualSnapshot).find((candidate) => candidate.id === '2');
  ...
  it('maps existing stations into occupied orbital and surface slots without turning them into planned placements', () => {
    ...
    const rows = buildPlannerCanvasRows(occupiedSystem, occupiedSnapshot);
    const body = rows.find((row) => row.id === '2');
    const summary = buildPlannerCanvasOccupancySummary(occupiedSystem, occupiedSnapshot);
    ...
    render(<SystemBuildMapCanvas system={occupiedSystem} snapshot={occupiedSnapshot} selection={{ type: 'body', bodyId: '2' }} onSelect={vi.fn()} onRequestAddStructure={vi.fn()} />);

    expect(screen.getByTestId('planner-canvas-existing-summary').textContent).toContain('Existing infrastructure detected');
    expect(screen.getAllByTestId('planner-canvas-existing-structure')).toHaveLength(2);
    expect(screen.getAllByTestId('planner-canvas-inferred-existing')).toHaveLength(2);
  ...
  it('renders confirmed backend associations in lane with explicit known marker', () => {
    ...
    const row = buildPlannerCanvasRows(confirmedSystem, confirmedSnapshot).find((candidate) => candidate.id === '2');
    ...
    render(<SystemBuildMapCanvas system={confirmedSystem} snapshot={confirmedSnapshot} selection={{ type: 'body', bodyId: '2' }} onSelect={vi.fn()} onRequestAddStructure={vi.fn()} />);

    expect(screen.getByTestId('planner-canvas-confirmed-existing').textContent).toContain('known');
    expect(screen.queryByTestId('planner-canvas-inferred-existing')).toBeNull();
  ...
  it('renders inferred backend associations in lane with explicit verify marker', () => {
    ...
    const row = buildPlannerCanvasRows(inferredSystem, inferredSnapshot).find((candidate) => candidate.id === '2');
    const summary = buildPlannerCanvasOccupancySummary(inferredSystem, inferredSnapshot);
    ...
    render(<SystemBuildMapCanvas system={inferredSystem} snapshot={inferredSnapshot} selection={{ type: 'body', bodyId: '2' }} onSelect={vi.fn()} onRequestAddStructure={vi.fn()} />);

    expect(screen.getByText('1 inferred')).toBeTruthy();
    expect(screen.getByTestId('planner-canvas-inferred-existing').textContent).toContain('verify');
  ...
  it('keeps transient backend unknown-lane associations out of lane capacity and unresolved warnings', () => {
    ...
    const rows = buildPlannerCanvasRows(unknownLaneSystem, { ...snapshot, placements: [], projection: null });
    const row = rows.find((candidate) => candidate.id === '2');

    expect(row?.existingCount).toBe(0);
    expect(row?.orbitalSlots[0].kind).toBe('empty');
    expect(buildPlannerCanvasOccupancySummary(unknownLaneSystem, { ...snapshot, placements: [], projection: null }).unresolvedExistingCount).toBe(0);
  });
  ...
  it('shows permanent unknown-lane infrastructure outside lane capacity', () => {
    ...
    const rows = buildPlannerCanvasRows(unknownLaneSystem, cleanSnapshot);
    const row = rows.find((candidate) => candidate.id === '2');
    const summary = buildPlannerCanvasOccupancySummary(unknownLaneSystem, cleanSnapshot);
    ...
    render(<SystemBuildMapCanvas system={unknownLaneSystem} snapshot={cleanSnapshot} selection={{ type: 'system' }} onSelect={vi.fn()} />);

    expect(screen.getByTestId('planner-canvas-unresolved-existing-structure').textContent).toContain('lane unknown');
  ...
  it('includes existing occupancy in add capacity and overflow calculations', () => {
    ...
    const fullState = getPlannerLaneCapacityState(oneExistingSystem, fullSnapshot, '2', 'orbital');
    const fullRow = buildPlannerCanvasRows(oneExistingSystem, fullSnapshot).find((row) => row.id === '2');
    ...
    const overflowRow = buildPlannerCanvasRows(oneExistingSystem, overflowSnapshot).find((row) => row.id === '2');
    expect(overflowRow?.orbitalSlots.map((slot) => slot.kind)).toEqual(['existing', 'planned', 'overflow']);

    render(<SystemBuildMapCanvas system={oneExistingSystem} snapshot={fullSnapshot} selection={{ type: 'body', bodyId: '2' }} onSelect={vi.fn()} onRequestAddStructure={vi.fn()} />);
  ...
  it('keeps Suggested Build projected occupancy ghost-only in lane capacity display', () => {
    ...
    const state = getPlannerLaneCapacityState(oneExistingSystem, ghostSnapshot, '2', 'orbital');
    const row = buildPlannerCanvasRows(oneExistingSystem, ghostSnapshot).find((candidate) => candidate.id === '2');
    const summary = buildPlannerCanvasOccupancySummary(oneExistingSystem, ghostSnapshot);
    ...
    render(<SystemBuildMapCanvas system={oneExistingSystem} snapshot={ghostSnapshot} selection={{ type: 'body', bodyId: '2' }} onSelect={vi.fn()} onRequestAddStructure={vi.fn()} />);
  ...
  it('shows unresolved permanent existing infrastructure compactly instead of forcing a body or lane', () => {
    ...
    render(<SystemBuildMapCanvas system={unresolvedSystem} snapshot={{ ...snapshot, placements: [], projection: null }} selection={{ type: 'system' }} onSelect={vi.fn()} />);

    const unresolved = screen.getByTestId('planner-canvas-unresolved-existing');
    ...
    expect(buildPlannerCanvasOccupancySummary(unresolvedSystem, { ...snapshot, placements: [], projection: null }).unresolvedExistingCount).toBe(1);
  });
  ...
  it('excludes fleet carriers and unconfirmed callsigns from unresolved planner warning counts', () => {
    ...
    const cleanSnapshot = { ...snapshot, placements: [], projection: null };
    const summary = buildPlannerCanvasOccupancySummary(transientSystem, cleanSnapshot);
    ...
    render(<SystemBuildMapCanvas system={transientSystem} snapshot={cleanSnapshot} selection={{ type: 'system' }} onSelect={vi.fn()} />);
    expect(screen.queryByTestId('planner-canvas-unresolved-existing')).toBeNull();
    ...
    expect(screen.getByTestId('planner-telemetry-warning-summary').textContent).toContain('0 unresolved existing');
  });

  it('excludes Barycentre and Null entries from the real system tree rows', () => {
    const rows = buildPlannerCanvasRows({
      ...system,
      bodies: [
        ...(system.bodies ?? []),
  ...
  it('renders real planner lanes without static mock bodies or attached-structure column', () => {
    ...
    expect(screen.getByTestId('planner-canvas-body-row-2')).toBeTruthy();
    ...
    expect(screen.getAllByTestId('planner-canvas-structure-economy').length).toBeGreaterThan(0);
  ...
  it('keeps lane display strict and leaves dual-location placements unassigned until a lane is known', () => {
    ...
    const unassignedRow = buildPlannerCanvasRows(system, flexibleSnapshot).find((row) => row.id === '2');
    ...
    const orbitalRow = buildPlannerCanvasRows(system, {
      ...flexibleSnapshot,
    ...
    const surfaceRow = buildPlannerCanvasRows(system, {
      ...flexibleSnapshot,
  ...
  it('renders zero-slot lanes compactly without fake empty slot boxes or add targets', () => {
    ...
    const rows = buildPlannerCanvasRows(system, zeroSlotSnapshot);
    const row = rows.find((candidate) => candidate.id === '2');
  ...
  it('highlights selected bodies without rendering an inline body planner', () => {
    ...
    expect(screen.getByTestId('planner-canvas-body-row-2').getAttribute('data-selected')).toBe('true');
    expect(screen.queryByTestId('planner-inline-body-expansion-2')).toBeNull();
  ...
  it('selects planned and projected structures from the main canvas', () => {
    ...
    render(<SystemBuildMapCanvas system={system} snapshot={snapshot} selection={{ type: 'system' }} onSelect={onSelect} />);
  ...
  it('keeps passive display slots from presenting button or hover affordances', () => {
    render(<SystemBuildMapCanvas system={system} snapshot={{ ...snapshot, placements: [], projection: null }} selection={{ type: 'system' }} onSelect={vi.fn()} />);
  ...
  it('shows disabled surface lane reasons directly in the canvas', () => {
    const onRequestAddStructure = vi.fn();
    render(<SystemBuildMapCanvas system={system} snapshot={{ ...snapshot, placements: [], projection: null }} selection={{ type: 'body', bodyId: '3' }} onSelect={vi.fn()} onRequestAddStructure={onRequestAddStructure} />);
  ...
  it('renders contextual station economy and prerequisite warnings on added structures', () => {
    ...
    render(<SystemBuildMapCanvas system={system} snapshot={contextualSnapshot} selection={{ type: 'placement', placementIndex: 0 }} onSelect={vi.fn()} />);

    expect(screen.getByTestId('planner-canvas-contextual-economy')).toBeTruthy();
    expect(screen.getByTitle(/Contextual - inherits from body\/system plan/i)).toBeTruthy();
    expect(screen.getByTestId('planner-canvas-prerequisite-warning')).toBeTruthy();
  });
  ...
  it('renders unpadded lane capacity chips and drops dot indicators on body rows (17N.1e)', () => {
    ...
    expect(screen.queryByTestId('planner-body-slot-indicators-2')).toBeNull();
    const bodyChip = screen.getByTestId('planner-canvas-body-capacity-2');
    expect(within(bodyChip).getByTestId('planner-canvas-body-capacity-2-orbital').dataset.capacity).toBe('2');
    expect(within(bodyChip).getByTestId('planner-canvas-body-capacity-2-ground').dataset.capacity).toBe('2');
  });
  ...
  it('shows passive empty slot boxes on unselected bodies instead of dot fallback (17N.1e)', () => {
    ...
    render(<SystemBuildMapCanvas system={system} snapshot={emptySnapshot} selection={{ type: 'system' }} onSelect={vi.fn()} />);
  ...
  it('calculates inherited baseline economy percentages for contextual stations placed over a body (17N.1e)', () => {
    ...
    render(<SystemBuildMapCanvas system={system} snapshot={baselineSnapshot} selection={{ type: 'placement', placementIndex: 0 }} onSelect={vi.fn()} />);

    expect(screen.getByTestId('planner-canvas-structure-economy')).toBeTruthy();
  ...
  it('does not surface slot/lane wording from prerequisites as a structure warning (17N.1e)', () => {
    ...
    const rows = buildPlannerCanvasRows(system, slotPrereqSnapshot);
    const body = rows.find((row) => row.id === '2');
  ...
  it('flags lane capacity overflow with capacity-exceeded warning copy (17N.1e)', () => {
    ...
    const rows = buildPlannerCanvasRows(system, overflowSnapshot);
    const body = rows.find((row) => row.id === '2');
  ...
  it('does not render visible PLAN or CTX labels on main canvas structure boxes (17N.1f)', () => {
    ...
    const allSlotPills = screen.getAllByTestId('planner-canvas-structure-pill');
    ...
    expect(screen.getByTestId('planner-canvas-contextual-economy')).toBeTruthy();
    expect(screen.getByTestId('planner-canvas-prerequisite-warning')).toBeTruthy();
    expect(screen.getByTestId('planner-canvas-contextual-economy').className).toContain('sr-only');
    expect(screen.getByTestId('planner-canvas-prerequisite-warning').className).toContain('sr-only');
  });

  it('renders economy micro-bars at a readable thickness and preserves tooltip (17N.1f)', () => {
    render(<SystemBuildMapCanvas system={system} snapshot={snapshot} selection={{ type: 'system' }} onSelect={vi.fn()} />);

    const bars = screen.getAllByTestId('planner-canvas-structure-economy');
  ...
  it('shows prominent selected row highlight and Add controls at readable size (17N.1f)', () => {
    ...
    const selectedRow = screen.getByTestId('planner-canvas-body-row-2');
  ...
  it('matches projected structures to large numeric system body ids', () => {
    ...
    const rows = buildPlannerCanvasRows(largeSystem, largeSnapshot);
    ...
    render(<SystemBuildMapCanvas system={largeSystem} snapshot={largeSnapshot} selection={{ type: 'system' }} onSelect={vi.fn()} />);

    expect(screen.getByTestId(`planner-canvas-body-row-${roundedBodyId}`).getAttribute('data-projected')).toBe('true');
    expect(screen.getByText('Ghost Refinery Hub')).toBeTruthy();
  });
  ...
  it('renders exact BGS telemetry deltas with positive and negative zero-centered bars from live planner state', () => {
    ...
    expect(screen.getByTestId('planner-telemetry-panel')).toBeTruthy();
    ...
    expect(screen.getByTestId('planner-stat-population-positive').dataset.tone).toBe('positive-green');
    expect(screen.getByTestId('planner-stat-security-negative').dataset.tone).toBe('negative-red');
    expect(screen.getByTestId('planner-stat-security-negative').style.width).not.toBe('0%');
    expect(screen.getByText('+72')).toBeTruthy();
    expect(screen.getByText('-20.9')).toBeTruthy();
    expect(screen.getByTestId('planner-development-profile-card')).toBeTruthy();
    expect(screen.getByTestId('planner-development-overall-score').textContent).toContain('72');
    expect(screen.getByText(/Refinery.*Industrial/)).toBeTruthy();
    expect(screen.getByText(/Extraction.*Refinery/)).toBeTruthy();
    expect(screen.getByText('68')).toBeTruthy();
    expect(screen.getByText('83')).toBeTruthy();
    expect(screen.getByText('18')).toBeTruthy();
    expect(screen.getByText('82%')).toBeTruthy();
    expect(within(screen.getByTestId('planner-telemetry-economy-ledger')).getByText(/Ind/i)).toBeTruthy();
    expect(screen.getByTestId('planner-projection-comparison')).toBeTruthy();
