...
export function PlannerCanvasPreview() {
  ...
  return (
    <section
      aria-label="Static colony planner preview"
      data-testid="planner-canvas-preview"
      className="space-y-3"
    >
      <header className="flex flex-col gap-3 border-b border-orange/20 bg-bg1/60 px-3 py-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-cyan">
            Preview - visual direction, not live planner data
          </div>
      ...
      <div
        data-testid="preview-planner-layout"
      ...
      >
        <main
          ...
          data-testid="planner-canvas-preview-grid"
        ...
        <aside
          data-testid="planner-canvas-preview-stats"
          data-layout="wide-telemetry"
          ...
          <div className="mt-4 border-t border-cyan/25 pt-3">
            ...
            <div className="mt-3 space-y-2 border-t border-cyan/15 pt-3">
              ...
              {selectedStructures.length > 0 ? selectedStructures.map((slot) => (
                ...
                <div className="font-mono text-[11px] text-silver-dk">No visible structures in this preview state.</div>
              )}
            </div>
          </div>
        </aside>
...
function CanvasBodyRow({
  ...
  return (
    <div
      data-testid={`preview-body-row-${node.id}`}
      className={[
        'transition-colors',
  ...
  return (
    <div className="relative min-h-full">
      ...
      <button
        type="button"
        onClick={onSelect}
        data-testid={`preview-body-select-${node.id}`}
