...
function warehousePlannerEvidenceResponse() {
  return {
    ...
    evidence_envelope: {
      ...
      summary: 'Selected-system evidence is available in this read-only planner envelope. Source classes: derived report evidence.',
    },
    bounded_staging: {
    ...
    coverage: {
      body_scan: {
        ...
        total_count: 2,
        coverage_ratio: 0.5,
        summary: '1/2 body scans are currently covered.',
      },
      station_links: {
        ...
        total_count: 0,
        coverage_ratio: null,
        summary: 'No station links are required for this selected system yet.',
      },
      ring_identity: {
        status: 'not_applicable',
        known_count: 0,
      ...
      thin_data_reasons: ['Body scan coverage is partial: 1/2 body scans are currently covered.'],
      summary: 'Coverage summary: 1/2 body scans are currently covered. No station links are required for this selected system yet. No ring-bearing bodies are currently known in canonical scan facts.',
    },
    evidence_summary: {
...
describe('ColonyPlannerWorkspace real planner passivity', () => {
  ...
  it('loads system context and passive planner data without running Preview or Suggested Builds', async () => {
    ...
    expect(screen.queryByTestId('planner-inline-body-expansion-body1')).toBeNull();
  ...
  it('keeps main row slot lanes aligned and updates them after explicit structure adds', async () => {
    ...
    await screen.findByTestId('planner-canvas-body-row-1');
    ...
    expect(screen.queryByTestId('planner-inline-body-expansion-1')).toBeNull();
    expect(screen.queryByTestId('selected-body-planner-canvas')).toBeNull();
    await waitFor(() => {
  ...
  it('adds structures directly from planner canvas slots without Preview, generation, or Advanced Planner dependency', async () => {
    mockedApiSystem.mockResolvedValue(slotMapSystem);
    mockedGetFacilityTemplates.mockResolvedValue(templates);
    mockedGetSimulationSummary.mockResolvedValue({
      classification: { primary_archetype: 'refinery_industrial' },
