    ...
    <ol className="space-y-1.5">
      {[...placements].sort((a, b) => a.build_order - b.build_order).map((placement) => (
        <li
          ...
          className="rounded-sm border border-border/50 bg-bg3/20 px-3 py-2 font-mono text-[11px] text-silver"
        >
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-orange">#{placement.build_order}</span>
            {placement.is_primary_port && (
              <span className="rounded-sm border border-cyan/30 bg-cyan/10 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.12em] text-cyan">
                Primary port
              </span>
            )}
