...
describe.each<RuntimeBackend>(['WEBGL2', 'WEBGPU'])('deterministic %s recovery harness', (backend) => {
  it('retains CPU state across loss/recovery and cleans up listeners', () => {
    ...
    expect(events.map((event) => event.type)).toEqual(['RESOURCE_LOST', 'RECOVERY_RESULT']);
    expect(events[1]).toMatchObject({ outcome: 'RECOVERED', latencyMs: expect.any(Number) });
  ...
  it('restores retained resources through exactly one upload callback', () => {
    const lost = new Signal(); const restored = new Signal(); let uploads = 0;
    const bridge = new ResourceRecoveryBridge({ lost, restored }, backend, () => { uploads += 1; }, () => undefined);
    bridge.attach(); lost.fire(); restored.fire();
