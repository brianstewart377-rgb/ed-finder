...
function resolvePython() {
  ...
  for (const candidate of candidates) {
    ...
    const probe = spawnSync(
      ...
      [
        ...prefixArgs,
        '-c',
        'import sys; (sys.implementation.name == "cpython" and sys.version_info[:2] == (3, 14)) or sys.exit(64); import asyncpg',
      ],
  ...
  throw new Error(
    'No usable CPython 3.14.x interpreter found for local OpenAPI generation. Install Python 3.14 with the pinned backend dependencies, or set ED_FINDER_PYTHON to that interpreter.',
  );
}
