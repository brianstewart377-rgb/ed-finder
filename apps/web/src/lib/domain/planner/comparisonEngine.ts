...
export function compareBuildSources(
  ...
  const placementComparison = comparePlacements(
    before.placements,
    after.placements,
  );
  const previewSummaryDelta = {
    final_score: numericDelta(
      before.previewSummary?.final_score,
      after.previewSummary?.final_score,
    ),
    composition_score: numericDelta(
      before.previewSummary?.composition_score,
      after.previewSummary?.composition_score,
    ),
    buildability_score: numericDelta(
      before.previewSummary?.buildability_score,
      after.previewSummary?.buildability_score,
    ),
    confidence: numericDelta(
      before.previewSummary?.confidence,
      after.previewSummary?.confidence,
    ),
  };
  const rankingDelta = compareRanking(before, after);
  const warningChanges = stringSetChanges(
    before.warnings ?? [],
    after.warnings ?? [],
  );
  const assumptionChanges = stringSetChanges(
    before.assumptions ?? [],
    after.assumptions ?? [],
  );
...
function compareRisk(
  ...
  if (warningCount.direction !== 'unknown' || cpKnown) {
    if (cpWorsened || warningCount.direction === 'worsened') {
      riskDirection = 'higher';
    } else if (cpImproved || warningCount.direction === 'improved') {
      riskDirection = 'lower';
    } else {
...
function totalWarningCount(source: BuildComparisonSource): number {
  return (
    ...
    (source.previewSummary?.warnings_count ?? 0)
  );
}

export function stringSetChanges(
  before: string[],
  after: string[],
): StringSetChanges {
...
function facilityCountDeltas(
  before: BuildComparisonSource['placements'],
  after: BuildComparisonSource['placements'],
): FacilityCountDelta[] {
...
function countByFacility(
  placements: BuildComparisonSource['placements'],
): Map<string, number> {
  const counts = new Map<string, number>();
  for (const placement of placements) {
    counts.set(
      placement.facility_template_id,
      (counts.get(placement.facility_template_id) ?? 0) + 1,
    );
  }
  return counts;
}
...
function comparePlacements(
  ...
  const beforeRemaining = before.map((placement, index) => ({
    placement,
    index,
    matched: false,
  }));
  const afterRemaining = after.map((placement, index) => ({
    placement,
    index,
    matched: false,
  }));
  ...
  for (const beforeItem of beforeRemaining) {
    const exact = afterRemaining.find(
      (afterItem) =>
        !afterItem.matched &&
        sameIdentity(beforeItem.placement, afterItem.placement),
    );
    if (!exact) continue;
    beforeItem.matched = true;
    exact.matched = true;
    const changeType = placementChangeType(
    ...
    if (changeType === 'unchanged') unchangedCount += 1;
      ...
      changed.push(
        changeFromPair(beforeItem.placement, exact.placement, changeType),
      );
    }
  }

  for (const beforeItem of beforeRemaining.filter((item) => !item.matched)) {
    const moved = afterRemaining.find(
      (afterItem) =>
        !afterItem.matched &&
        beforeItem.placement.facility_template_id ===
          afterItem.placement.facility_template_id,
    );
    if (!moved) continue;
    beforeItem.matched = true;
    moved.matched = true;
    changed.push(
      changeFromPair(beforeItem.placement, moved.placement, 'body_changed'),
    );
  }
...
function sameIdentity(
  a: BuildComparisonSource['placements'][number],
  b: BuildComparisonSource['placements'][number],
): boolean {
  return (
    a.facility_template_id === b.facility_template_id &&
    (a.local_body_id ?? null) === (b.local_body_id ?? null)
  );
}
...
function placementChangeType(
  ...
  if (Boolean(before.is_primary_port) !== Boolean(after.is_primary_port)) {
    return 'primary_port_changed';
  }
...
function sortChanges(changes: PlacementChange[]): PlacementChange[] {
  return [...changes].sort((a, b) => {
    const facility = a.facility_template_id.localeCompare(
      b.facility_template_id,
    );
    if (facility !== 0) return facility;
    return (
      (a.after_build_order ?? a.before_build_order ?? 0) -
      (b.after_build_order ?? b.before_build_order ?? 0)
    );
  });
}

function primaryPortChange(
  before: BuildComparisonSource['placements'],
  after: BuildComparisonSource['placements'],
  ...
  if (beforePort && afterPort && sameIdentity(beforePort, afterPort)) {
    ...
    return changeType === 'unchanged'
      ? null
      : changeFromPair(beforePort, afterPort, changeType);
  }
  return {
    facility_template_id:
      afterPort?.facility_template_id ??
      beforePort?.facility_template_id ??
      'unknown',
...
function buildTradeoffSummary(
  result: Omit<BuildComparisonResult, 'tradeoff_summary' | 'recommendation'>,
): string[] {
  const summary: string[] = [];
  if (result.target_archetype_changed) {
    summary.push(
      `Changes target archetype from ${result.before_target_archetype ?? 'unknown'} to ${result.after_target_archetype ?? 'unknown'}.`,
    );
  }
  if (
    result.added_facilities.length > 0 ||
    result.removed_facilities.length > 0
  ) {
    summary.push(
      `Adds ${result.added_facilities.length} ${plural('facility', result.added_facilities.length)} and removes ${result.removed_facilities.length} ${plural('facility', result.removed_facilities.length)}.`,
    );
  }
  if (result.primary_port_change) {
    summary.push(
      `Moves primary port from ${result.primary_port_change.before_body_id ?? 'none'} to ${result.primary_port_change.after_body_id ?? 'none'}.`,
    );
  }
  const finalScore = result.preview_summary_delta.final_score;
  if (finalScore.direction === 'improved') {
    summary.push(
      `Improves final score by ${formatNumber(finalScore.delta)} points.`,
    );
  }
  if (finalScore.direction === 'worsened') {
    summary.push(
      `Reduces final score by ${formatNumber(Math.abs(finalScore.delta ?? 0))} points.`,
    );
  }
  const warningDelta = result.risk_delta.warning_count.delta ?? 0;
  if (result.risk_delta.warning_count.direction === 'improved') {
    summary.push(`Reduces warning count by ${Math.abs(warningDelta)}.`);
  }
  if (result.risk_delta.warning_count.direction === 'worsened') {
    summary.push(`Increases warning count by ${Math.abs(warningDelta)}.`);
  }
  if (
    result.risk_delta.before_cp_negative === false &&
    result.risk_delta.after_cp_negative === true
  ) {
    summary.push('Introduces CP risk.');
  }
  if (
    result.risk_delta.before_cp_negative === true &&
    result.risk_delta.after_cp_negative === false
  ) {
    summary.push('Removes CP risk.');
  }
  if (summary.length === 0) {
    summary.push('No major placement changes detected.');
  }
...
function buildRecommendation(
  result: Omit<BuildComparisonResult, 'tradeoff_summary' | 'recommendation'>,
): BuildComparisonResult['recommendation'] {
  ...
  if (
    ...
    (riskDirection === 'unknown' || riskDirection === 'unchanged')
  ) {
    return {
      verdict: 'insufficient_data',
      reasons: ['No score, ranking, or risk signal is available.'],
    };
  }
  if (
    (scoreDirection === 'improved' || rankDirection === 'improved') &&
    riskDirection !== 'higher'
  ) {
    reasons.push(
      scoreDirection === 'improved'
        ? 'Final score improves without higher risk.'
        : 'Ranking signal improves without higher risk.',
    );
    return { verdict: 'prefer_after', reasons };
  }
  if (
    scoreDirection === 'worsened' ||
    (riskDirection === 'higher' && scoreDirection !== 'improved')
  ) {
    reasons.push(
      scoreDirection === 'worsened'
        ? 'Final score worsens.'
        : 'Risk increases without a final-score improvement.',
    );
    return { verdict: 'prefer_before', reasons };
  }
  if (
    (scoreDirection === 'improved' && riskDirection === 'higher') ||
    result.preview_summary_delta.confidence.direction === 'worsened'
  ) {
    reasons.push(
      scoreDirection === 'improved' && riskDirection === 'higher'
        ? 'Score improves, but risk also increases.'
        : 'Confidence worsens, so the tradeoff is mixed.',
    );
    return { verdict: 'mixed', reasons };
  }
  return {
    verdict: 'mixed',
    reasons: ['Available signals do not clearly prefer either plan.'],
  };
}
