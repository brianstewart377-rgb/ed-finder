...
export function buildExportArtifacts({
  ...
  const plannedRows = placements
    ...
    .map((placement, index) => {
      const template = templates.find(
        (item) => item.id === placement.facility_template_id,
      );
      const body = placement.local_body_id
        ? bodies.find(
            (item) => String(item.id) === String(placement.local_body_id),
          )
        : null;
      return {
  ...
  const operatorReviewFocusItems = [
    ...(reasons.length > 0
      ? reasons
      : ['Current planner/export state has no closeout blockers.']),
  ...
  const operatorReview = {
    ready:
      closeoutReady && provenance?.evidence_panels.warehouse.state !== 'stale',
    ...
    references: {
      ...
      source_run_key:
        provenance?.provenance_summary?.latest_source_run_key ?? null,
      artifact_name:
        provenance?.evidence_panels.source_run?.artifact_name ?? null,
  ...
  const payload = {
    ...
    warehouse: provenance
      ? {
          ...
          stale_records:
            provenance.evidence_panels.warehouse.stale_records ?? null,
        }
      : null,
  ...
  const markdown = [
    ...
    plannedRows.length > 0
      ? plannedRows
          .map(
            (row) =>
              `- ${row.step}. ${row.facility_name} (${row.body_name || 'Unassigned'})${row.is_primary_port ? ' [primary port]' : ''}`,
          )
          .join('\n')
      : `- No placements`,
    ``,
    ...
    ...Object.entries(observedByType).map(
      ([factType, count]) => `- ${factType}: ${count}`,
    ),
    ``,
    `## Inferred`,
    ...
    `## Guardrails`,
    provenance
      ? Object.entries(provenance.guardrails)
    ...
    ...governance.documentation_references.map(
      (item) => `- Reference: ${item}`,
    ),
    ``,
  ...
  const csvLines = [
    ...
    ...plannedRows.map((row) =>
      [
        row.step,
        row.facility_template_id,
        escapeCsv(row.facility_name),
        escapeCsv(row.body_name),
        row.is_primary_port ? 'true' : 'false',
      ].join(','),
    ),
  ];
