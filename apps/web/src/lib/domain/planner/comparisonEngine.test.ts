...
import type { BuildComparisonSource } from './comparisonTypes';
...
function source(
  overrides: Partial<BuildComparisonSource> = {},
...
describe('Stage 5E comparison engine', () => {
  ...
  it('detects added removed and facility count deltas', () => {
    const result = compareBuildSources(
      source({
        placements: [
          placement('port_a', 'body1', 1, true),
          placement('support_z', 'body1', 2),
        ],
      }),
      after(),
    );
    expect(
      result.added_facilities.map((item) => item.facility_template_id),
    ).toContain('support_y');
    expect(
      result.removed_facilities.map((item) => item.facility_template_id),
    ).toContain('support_z');
    expect(result.facility_count_deltas).toEqual(
      expect.arrayContaining([
        {
          facility_template_id: 'support_y',
          before_count: 0,
          after_count: 1,
          delta: 1,
        },
        {
          facility_template_id: 'support_z',
          before_count: 1,
          after_count: 0,
          delta: -1,
        },
  ...
  it('detects body assignment build order and primary port changes', () => {
    const result = compareBuildSources(
      ...
      after({
        placements: [
          placement('port_a', 'body2', 2, true),
          placement('support_x', 'body1', 1, true),
        ],
      }),
    );
    expect(
      result.changed_placements.some(
        (item) =>
          item.change_type === 'body_changed' &&
          item.facility_template_id === 'port_a',
      ),
    ).toBe(true);
    expect(
      result.changed_placements.some(
        (item) =>
          item.change_type === 'primary_port_changed' &&
          item.facility_template_id === 'support_x',
      ),
    ).toBe(true);
    expect(result.primary_port_change?.change_type).toBe(
  ...
  it('detects order changes for identical facility/body placements', () => {
    ...
    expect(result.changed_placements).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          facility_template_id: 'support_x',
  ...
  it('handles duplicate facilities deterministically and does not mutate inputs', () => {
    ...
    expect(
      first.changed_placements.some(
        (item) => item.change_type === 'order_changed',
      ),
    ).toBe(true);
  });
  ...
  it('computes preview summary and ranking deltas', () => {
    ...
    expect(result.preview_summary_delta.composition_score.direction).toBe(
      'improved',
    );
    expect(result.preview_summary_delta.buildability_score.direction).toBe(
      'improved',
    );
  ...
  it('reports warning and assumption set changes without duplicate noise', () => {
    expect(
      stringSetChanges([' Alpha ', 'Alpha', 'Beta'], ['Beta', 'Gamma']),
    ).toEqual({
      before: ['Alpha', 'Beta'],
      after: ['Beta', 'Gamma'],
      added: ['Gamma'],
  ...
  it('builds deterministic tradeoff summaries and conservative recommendations', () => {
    ...
    expect(improved.tradeoff_summary).toContain(
      'Changes target archetype from refinery_industrial to agriculture_terraforming.',
    );
    expect(
      improved.tradeoff_summary.some((item) =>
        item.includes('Improves final score'),
      ),
    ).toBe(true);
    ...
    const noChange = compareBuildSources(
      source({
        placements: [placement('support_x', 'body1', 1)],
        previewSummary: null,
        ranking: null,
        warnings: [],
      }),
    ...
    expect(noChange.tradeoff_summary).toEqual([
      'No major placement changes detected.',
    ]);
  });
  ...
  it('creates copied sources from optimiser candidates and current preview placements', () => {
    ...
    const sourceFromPreview = sourceFromCurrentPreview({
  ...
  it('formats deltas verdicts risks and placement changes without React dependencies', () => {
    expect(
      formatDeltaValue(
        { before: 1, after: 2.5, delta: 1.5, direction: 'improved' },
        ' pts',
      ),
    ).toBe('+1.5 pts');
    expect(
