...
export function recommendedBodyId(
  ...
  const locationText =
    `${location ?? ''} ${template.allowed_location}`.toLowerCase();
...
export function archetypeFromEconomy(economy?: string | null): string | null {
  ...
  if (
    normalised.includes('hightech') ||
    normalised.includes('high tech') ||
    normalised.includes('tourism')
  ) {
    return 'hitech_tourism';
  }
