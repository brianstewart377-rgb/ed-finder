"""Ring source payload normalisation.

The helpers here preserve unknown/missing ring evidence as unknown. They only
produce explicit no-ring evidence when the caller states that the source payload
is a trusted full body scan and the source included an empty ring array.
"""
...
def normalise_ring_payload(payload: dict[str, Any], *, trusted_empty_means_no_rings: bool = False) -> RingNormalisation:
    ...
    if not raw_entries:
        return RingNormalisation(
            rings=[],
            explicit_no_rings=trusted_empty_means_no_rings,
            ring_array_present=True,
        )

    rings = [_normalise_ring_entry(entry) for entry in raw_entries]
    return RingNormalisation(rings=rings, explicit_no_rings=False, ring_array_present=True)
...
def ring_rows_for_body(
    ...
    *,
    ...
    result = normalise_ring_payload(
        payload,
        trusted_empty_means_no_rings=trusted_empty_means_no_rings,
    )
...
def _normalise_ring_entry(entry: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        'inner_radius': _safe_float(_first_present_key(entry, (
            'innerRadius', 'InnerRadius', 'InnerRad', 'innerRad', 'inner_radius', 'inner_rad',
        ))),
        'outer_radius': _safe_float(_first_present_key(entry, (
            'outerRadius', 'OuterRadius', 'OuterRad', 'outerRad', 'outer_radius', 'outer_rad',
        ))),
    }
