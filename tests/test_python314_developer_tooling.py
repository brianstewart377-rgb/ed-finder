...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("scripts", "dev", "start_local_api.ps1"),
...
def test_windows_bootstrap_selects_and_verifies_exact_cpython314():
    ...
    assert "repository virtualenv must use CPython 3.14.x" in source
...
def test_windows_doctor_fails_closed_on_non_cpython314():
    ...
    assert source.count("$runtime -eq 'cpython:3.14'") == 4
...
def test_windows_api_launcher_verifies_existing_virtualenv_runtime():
    source = _read("scripts", "dev", "start_local_api.ps1")

    assert "$venvRuntime -ne 'cpython:3.14'" in source
    assert "repository virtualenv must use CPython 3.14.x" in source
    assert source.index("$venvRuntime -ne 'cpython:3.14'") < source.index("-m uvicorn")
...
def test_react_openapi_generator_only_accepts_cpython314():
    ...
    assert "platform.python_implementation() == 'CPython'" in source
    assert "sys.version_info[:2] == (3, 14)" in source
    assert "No usable CPython 3.14 interpreter found" in source
...
def test_shell_developer_launchers_prefer_and_verify_cpython314(script_name: str):
    source = _read("scripts", "checks", script_name)

    assert "command -v python3.14" in source
    assert source.index("elif command -v python3.14") < source.index("elif command -v python3 >")
    assert "platform.python_implementation() == 'CPython'" in source
    assert "sys.version_info[:2] == (3, 14)" in source
