...
def test_optional_postgres_staging_loader_smoke(tmp_path):
    source_file = tmp_path / f'edsm_station_snapshot_{uuid.uuid4().hex}.json'
    shutil.copyfile(FIXTURE, source_file)
    source_run_key = None

    with db_loader.connect_staging_db(DSN) as conn:
        preflight = db_loader.check_staging_schema(conn)
        ...
        try:
            ...
            assert write_report['summary']['staging_station_rows_written'] == 2
            ...
            assert staged_report['summary']['staged_station_rows'] == 2
            assert staged_report['summary']['warning_records'] == 0
            assert staged_report['summary']['error_records'] == 0
            assert staged_report['source_run']['source'] == 'edsm_nightly_stations'
            assert staged_report['source_files'][0]['source_file_key'] == source_file_key
        finally:
            if source_run_key is not None:
                cleanup_staged_run(conn, source_run_key)


def cleanup_staged_run(conn, source_run_key: str) -> None:
    cleanup_sql = [
        """
