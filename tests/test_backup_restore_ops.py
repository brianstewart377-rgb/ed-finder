...
def test_backup_helpers_remain_in_legacy_maintenance_compose_without_v2_storagebox_authority():
    ...
    assert dockerfile.startswith('FROM python:3.14-alpine3.22\n')
    assert 'apk add --no-cache dcron tini bash postgresql16-client rclone curl' in dockerfile
    assert 'python3 -m pip install --no-cache-dir "psycopg==3.3.4"' in dockerfile
