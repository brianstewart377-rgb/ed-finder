...
def test_frontend_target_remains_compatible_with_review_api():
    ...
    assert 'Only the two bounded viewport diagnostics owned by PR #259 are allowlisted' in docs
    assert 'planner_constrained_layout_compromise_diagnostic' in docs
    assert 'planner_mobile_resilience_overflow_diagnostic' in docs
    assert 'environment_ready: true' in docs
    assert 'product_acceptance_ready: true' in docs
    assert "'cypress/e2e/release-gate.cy.js'" in cypress_config
    assert "'cypress/e2e/auth-owner-access.cy.js'" in cypress_config
    ...
    assert "path.dirname(outputPath), 'cypress'" in review_cypress_config
    assert "--force-device-scale-factor=1" in review_cypress_config
    assert 'Browser resource load failed:' in review_spec
    assert "securitypolicyviolation" in review_spec
...
def test_review_lab_workflow_uses_failure_only_sanitised_artifacts_and_summary():
    ...
    assert '/tmp/edfinder-local-review/*/cypress' in workflow
...
def test_browser_result_card_expansion_helper_is_idempotent():
    ...
    assert "details[0]?.querySelector('summary')" in source
    assert 'summaryNode.click();' in source
    assert "cy.wrap(details).find('summary').click();" not in source
...
@pytest.mark.unit
def test_browser_originated_resource_error_fails_browser_console_phase():
    phase = review_env.evaluate_browser_console(
        {
            'apiResponses': [],
            'consoleEntries': [
                {
                    'type': 'error',
                    'text': 'Browser resource load failed: http://127.0.0.1:4173/missing.css',
                },
            ],
            'pageErrors': [],
        }
