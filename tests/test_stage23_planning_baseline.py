...
def test_stage23_authority_activates_the_next_post22_control_baseline():
    ...
    assert stage23['status'] == 'evidence_envelope_governance_completed'
    ...
    assert stage23['current_checkpoint'] == 'Stage 23C - Evidence envelope governance and source semantics'
    assert stage23['next_checkpoint'] == 'Stage 23D - Read-only planner evidence UX follow-through'
...
def test_stage23_docs_readme_and_stage17p_make_the_new_control_order_explicit():
    ...
    assert 'Stage 23D - Read-only planner evidence UX follow-through' in roadmap
