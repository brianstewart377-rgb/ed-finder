"""Fail-closed repository contracts for the consolidated V3 Svelte platform."""
...
def test_legacy_admin_token_allowlist_matches_require_admin_decorators():
    ...
    assert len(inventory_entries) == len(set(inventory_entries))
    assert set(inventory_entries) == backend_routes
...
def test_canonical_v3_journey_stays_explore_inspect_plan_review():
    matcher = (SOURCE / "params" / "journey.ts").read_text(encoding="utf-8")
    routes = SOURCE / "routes"
    shell = (SOURCE / "lib" / "components" / "AppShell.svelte").read_text(
        encoding="utf-8"
    )

    assert "new Set(['explore', 'inspect', 'plan', 'review'])" in matcher
    assert not (routes / "finder").exists()
    assert not (routes / "system").exists()
    assert not (routes / "colony-planner").exists()
    assert "page.url.pathname === '/explore' ? urlSelection : null" in shell
...
def test_root_layout_owns_the_configured_query_and_persistence_singletons():
    ...
    context = (SOURCE / "lib" / "persistence" / "context.ts").read_text(
        encoding="utf-8"
    )
    ...
    assert "Persistence context has not been provided" in context
    ...
    assert owners == ["routes/+layout.svelte"]
