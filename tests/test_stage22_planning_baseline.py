...
def test_stage22_authority_prepares_the_post182021_control_baseline():
    ...
    assert stage22['status'] == 'prepared'
    assert stage22['planning_authorized'] is True
    assert stage22['implementation_started'] is False
    assert stage22['implementation_authorized'] is False
    assert stage22['primary_objective'] == PRIMARY_OBJECTIVE
    assert stage22['first_executable_checkpoint'] == FIRST_CHECKPOINT
    assert stage22['current_checkpoint'] == 'Stage 22 planning baseline'
    assert stage22['next_checkpoint'] == FIRST_CHECKPOINT
