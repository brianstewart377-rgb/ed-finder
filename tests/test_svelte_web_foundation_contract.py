...
def test_svelte_transport_stays_isolated_from_the_legacy_numeric_react_client():
    source = "\n".join(
        path.read_text(encoding="utf-8")
        for path in (WEB / "src").rglob("*")
        if path.is_file() and path.suffix in {".js", ".mjs", ".svelte", ".ts"}
    )
    client = _read("apps", "web", "src", "lib", "api", "client.ts")

    assert "@ed-finder/api-client" not in source
    assert "packages/api-client" not in source
    assert "jsonFetch" not in client
