...
def test_legacy_compose_and_maintenance_are_not_v3_pg18_authority():
    ...
    assert 'PostgreSQL 16 client matches the retained root Compose environment only' in maintenance
    assert 'Current V3' in maintenance and 'PostgreSQL 18' in maintenance
    assert maintenance.startswith('FROM python:3.14-alpine3.22\n')
