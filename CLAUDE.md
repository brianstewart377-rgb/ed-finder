...
- Every bug fix ships with a contract/regression test if one could have caught the bug. Fix-only commits without hardening are incomplete — the test is part of the fix, not a follow-up.
- **Every PR must satisfy the canonical [Pull Request Acceptance Policy](docs/development/pull-request-acceptance-policy.md) before merge.** Acceptance is fail-closed and applies only to evidence for the exact latest PR head SHA. Green CI alone is insufficient: all protected checks and both Codex Review (`chatgpt-codex-connector`) and Octopus Review must satisfy the policy, every paginated review surface must be inspected, and every substantive finding must be dispositioned. Pending or still-reviewing services must be triggered when needed and awaited; only actual service failure or unavailability can enter the policy's explicit owner-waiver path.
...
- Automatic Dependabot auto-merge is disabled fail-closed. It may be reintroduced only through a separate enforceable mechanism that proves the complete canonical [Pull Request Acceptance Policy](docs/development/pull-request-acceptance-policy.md), rather than treating CI alone as sufficient acceptance evidence.
