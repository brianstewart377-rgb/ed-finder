...
- Every bug fix ships with a contract/regression test if one could have caught the bug. Fix-only commits without hardening are incomplete — the test is part of the fix, not a follow-up.
- **Every PR must satisfy the canonical [Pull Request Acceptance Policy](docs/development/pull-request-acceptance-policy.md) before merge.** Required checks and both configured reviewers — `chatgpt-codex-connector` (Codex Review) and Octopus Review — must complete successfully against the recorded latest-head SHA; green CI alone is insufficient. Inspect every paginated review surface and resolve or explicitly disposition every substantive finding. Reviewer failures are fail-closed except for the policy's explicit written owner-waiver path. Bot findings are evidence, not oracles: verify each claim against the actual latest code (trace values to where they are persisted or used, search the whole repository before calling code unused, and read the relevant tests before claiming a gap).
...
- Automatic Dependabot auto-merge is disabled fail-closed. It may be reintroduced only after a separate enforceable mechanism verifies the complete canonical latest-head acceptance evidence: all required checks, both Codex Review and Octopus Review on that exact SHA, paginated inspection of every review surface, and recorded dispositions for every finding. CI alone is never enough.
