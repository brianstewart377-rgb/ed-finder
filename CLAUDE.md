...
ED-Finder: an Elite Dangerous colonisation planner. **Stage status (this line is a summary of `docs/ROADMAP.md`, which is the authority — keep it in sync after every lane change):** Stages 25A–25H and **26A–26E are complete**. **Stage 27A Spatial Platform Contract and Audit is COMPLETE / ACCEPTED, and its contracts remain normative. Stage 27B Babylon 9 Runtime Workbench is CURRENT / AUTHORIZED as an isolated workbench only, with no production map wiring or cutover.** The Stage 26 R3F/Three.js desktop map (cutover commit `3b53477`) remains production and rollback (`VITE_STAGE26E_PRODUCTION_MAP=disabled`) until the later Stage 27G measured bakeoff explicitly earns cutover. Stage 27 may support **Explore → Inspect → Plan → Review** spatially, but Colony Planner/Cockpit remains the canonical detailed Build Plan workspace/persistence owner. No renderer owns planning logic or mechanics, and no map action may silently mutate a Build Plan or execute Preview.
...
**Current lane:** Stage 27B Babylon 9 Runtime Workbench, isolated from every
production route and flag. Stage 27A is complete and owner-accepted.

**Active focus:** Implement only the Babylon 9-class runtime workbench under
the accepted Stage 27A product, architecture, data-readiness, migration,
governance and cross-repo contracts. Do not wire a production route or flag,
modify or remove the production R3F baseline, cut over, deploy, touch production
data/database or V2, or begin 27C or any later stage.
...
**Lane boundary:** Stage 27B must stop before Stage 27C. R3F/Three remains the
production map and rollback until Stage 27G's measured bakeoff and explicit
cutover decision.
...
- Spatial-platform work is authorized only through `docs/ROADMAP.md` and the
  Stage 27 contracts. Stage 27A is complete/accepted and its contracts remain
  normative; only isolated Stage 27B work is authorized now.
