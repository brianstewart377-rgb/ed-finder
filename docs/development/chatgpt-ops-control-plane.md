...
Every active investigation and implementation execution uses one trusted model contract: the workflow passes the literal official model ID `gpt-5.6-sol` with `--model` and the literal `model_reasoning_effort="high"` override with `--config`. `high` is the normal Contabo self-hosted-pool default. These values are workflow authority, not request fields. The worker ignores ambient user configuration, enables strict configuration validation, and checks the installed CLI help for all required options before execution; an older or incompatible CLI therefore fails before the task prompt is sent instead of silently using another model or effort. `max` is exceptional and requires a separate owner-approved governance change; task text cannot select it or any other model or reasoning-effort override.

Immediately before each execution, the worker prints a sanitised attestation containing the bounded Codex CLI version, fixed model and reasoning effort, bounded request ID, mode, actual selected branch, and immutable base SHA. An implementation additionally exposes its sealed result SHA in normal wrapper output and, after the trusted push succeeds, records the branch and result SHA in the GitHub job summary.
