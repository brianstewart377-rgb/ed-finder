...
Every active investigation and implementation execution uses one trusted model contract: the workflow passes the literal official model ID `gpt-5.6-sol` with `--model` and the literal `model_reasoning_effort="high"` override with `--config`. These values are workflow authority, not request fields. The worker ignores ambient user configuration, enables strict configuration validation, and checks the installed CLI help for all required options before execution; an older or incompatible CLI therefore fails before the task prompt is sent instead of silently using another model or effort. The fixed `high` effort is the only resource-consumption adjustment; it does not relax the immutable model choice or any execution, repository-write, CI, review, or deployment boundary.
...
1. A small GitHub-hosted **prepare job** validates mode and branch routing before Codex exists. It refuses `main`, `master`, `codex-task-requests`, `chatgpt-ops-requests`, `chatgpt-ed-new-ops-requests`, and `chatgpt-v3-checkpoint-requests`; captures the exact target/base SHA; and emits only validated branch/SHA metadata. Arbitrary task text is never written to `$GITHUB_OUTPUT`.
