...
Every active investigation and implementation execution uses one trusted model contract: the workflow passes the literal official model ID `gpt-5.6-sol` with `--model` and the literal `model_reasoning_effort="max"` override with `--config`. These values are workflow authority, not request fields. The worker ignores ambient user configuration, enables strict configuration validation, and checks the installed CLI help for all required options before execution; an older or incompatible CLI therefore fails before the task prompt is sent instead of silently using another model or effort.
