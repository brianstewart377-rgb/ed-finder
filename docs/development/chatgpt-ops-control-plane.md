...
The self-hosted **Codex job has no repository contents permission**. It downloads the source artifact, deletes and recreates its workspace, initializes a fresh local repository, imports the source bundle, verifies the sealed SHAs, and deliberately configures **no network Git remote**. Before its first Git invocation it creates a per-run temporary HOME/XDG config root, disables system/global Git configuration and system attributes, and disables hooks. This prevents persistent `~/.gitconfig`, `core.attributesFile`, smudge filters, or hook configuration left by an earlier Codex run from executing while the trusted bundle is materialized.
