...
The real-service pair is `tests/test_stage19_real_postgres_readiness.py`. It
uses live Postgres through Psycopg 3, runs in read-only transaction mode, checks
`SELECT 1`, and verifies the approved Stage 19AR baseline identity when that
canonical baseline is present locally. It skips explicitly for absent
credentials, absent service, or absent baseline data and does not fall back to
fakes.
