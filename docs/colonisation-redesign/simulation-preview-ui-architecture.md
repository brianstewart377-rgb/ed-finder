...
Stage 6C does **not** touch the frontend. The Stage 6B Observed Evidence panel, `SimulationPreview.tsx`, `useSimulationPreviewRun`, `useSimulationPreviewPlan`, `OptimiserCandidatePanel`, `ColonyPlannerSectionNav.tsx`, and every Preview Result rendering component remain unchanged. Stage 6D is reserved for the frontend validation UI that will call `POST /api/observations/compare` from inside Simulation Preview and render the comparison summary, per-row statuses, severities, evidence shelf, and recommended actions returned by the Stage 6C engine. Stage 6D consumption rules will be specified before any rendering work begins; until then the engine is exposed only through the new endpoint and is exercised by `tests/test_stage6c_comparison.py`.
