...
the system-scoped identity concept `{ systemId64, bodyId }`. This target shape
does not assert that SQL `bodies.id` equals a journal or other source BodyID;
adapter/source semantics must be proven, names remain labels, and unresolved
