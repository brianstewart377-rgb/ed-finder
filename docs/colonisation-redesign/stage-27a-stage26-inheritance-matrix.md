...
This completed Stage 27A audit records what the V3 spatial platform can inherit
from Stages 25 and 26. It is historical audit/evidence input, not a current
roadmap, implementation gate, or production authority.
...
React/R3F/Three references below remain historical or baseline Stage 26
evidence. Future application/domain orchestration and accessible DOM ownership
belongs to Svelte/SvelteKit; that framework amendment neither rewrites the
Stage 26 result nor changes the renderer-neutral boundary.
...
|---|---|---:|---|
| Product role | [`stage-25-roadmap.md`](../archive/stage-25/stage-25-roadmap.md) §§ Map Product Decision/Stage 25G and [`stage-26a-next-generation-map-foundation-contract.md`](../archive/stage-26/stage-26a-next-generation-map-foundation-contract.md) § Product Decision constrain Map to secondary Explore. | **SUPERSEDED** | Spatial interaction may support Explore → Inspect → Plan → Review. Colony Planner/Cockpit remains the canonical detailed Build Plan workspace and persistence owner; no silent Build Plan mutation or Preview execution is permitted. |
| Renderer history | [`stage-26b-renderer-bakeoff-decision.md`](../archive/stage-26/stage-26b-renderer-bakeoff-decision.md) §§ Equal Matrix/Measurement Summary/Rationale selects R3F after the equal bakeoff; [`stage-26e-cutover-readiness.md`](../archive/stage-26/stage-26e-cutover-readiness.md) §§ Status/Production Activation Receipt records commit `3b53477` serving it publicly in the V2 era. | **HISTORICAL_ONLY** | Preserve as decision and migration evidence. React/R3F/Three does not define the V3 target or current production renderer. |
...
| Exploration / Commander History | `ProductionMapOverlays.explorationVisits/explorationTrail`; `useExplorationLayers.ts`; `explorationGeometry.ts`; `RouteDetailPanel.tsx`; exploration facts/projections/APIs. | **EXPAND** | Commander History is the shared personal-history backbone; Exploration remains a first-class interpretation. Retain chronology and route geometry, add explainable scan/FSS/DSS/bio/Codex facts, and preserve a useful non-map Journal page. Galaxy answers where; System answers what/body; Journal answers history/statistics/records/filters/expeditions. Personal evidence is never universal truth. |
| Journal ↔ spatial queries | No Stage 26 equivalent beyond one-way Exploration overlays. | **EXPAND** | Identified Journal results emit renderer-neutral contributions; viewport/selection/route scopes can request bounded personal summaries. Missing identity or coverage remains unresolved. No separate Journal map. |
| Finder × Journal | Finder and Exploration currently have separate feature paths. | **EXPAND** | Finder owns query/ranking; Commander History owns personal predicates/facts; renderer visualises results. Compose without duplicated logic or silent personal/public merging. |
...
| Accessibility | Stage 26A requirements 10–11; Stage 26C keyboard companion; Stage 26E §§ Closed Engineering Evidence; `R3FMapFoundation.test.tsx` keyboard/focus/reduced-motion tests and `ProductionMapTab.tsx` accessible companion UI. | **KEEP** | Svelte/SvelteKit owns accessible DOM UI, including keyboard/text, named controls, companion lists, overlap choices and explanations. Preserve shortcut disablement, form/dialog non-interference, focus stability, reduced motion and non-canvas equivalents. Extend to System scene. |
...
| Rollback/cutover | The archived [`stage-26e-cutover-readiness.md`](../archive/stage-26/stage-26e-cutover-readiness.md) and `production-route-flag.ts` retain the V2-era rollback receipt at commit `3b53477`. | **HISTORICAL_ONLY** | Retain useful parity and rollback lessons as migration evidence. Current V3 checkpoint/promotion requirements come from the roadmap, browser authority, and infrastructure status. |
...
| `explorationGeometry.test.ts` plus future Journal contracts | **ADAPT** | Retain chronology, distinct partial/complete states and pick-ID round-trip; expand to standalone history analytics, raw-to-projection replay/idempotency, body identity, timestamps, personal/global Codex, exobiology edge cases, reverse spatial queries and Finder composition. |
...
| `production-route-flag.test.ts` | **HISTORICAL** | Preserve its exact Stage 26/V2 enabled/disabled semantics as migration evidence. Any V3 promotion needs a separately reviewed release/rollback contract, not an in-place reinterpretation. |
...
| Exploration/Commander-History-rich | Personal chronological scan/FSS/DSS/bio/Codex facts, records and explainable counts; prove Journal works without a map and personal state is not universal truth. Synthetic fixtures are required; opt-in privacy-safe real logs are supplemental only and never public without explicit approval. |
...
all four architecture stress tests: Finder, Colonisation, Exploration/Journal
(with Commander History as backbone), and System Map. No Stage 26 test may be
removed merely because its implementation
