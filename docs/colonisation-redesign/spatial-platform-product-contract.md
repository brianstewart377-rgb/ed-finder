...
supersede current Colony Planner/Cockpit ownership of the detailed Build Plan
workspace and persistence, or the existing mechanics ownership boundaries.
...
`BodyRef` is the renderer-neutral target shape. The meaning of `bodyId` must be
proven for each adapter/source and is always interpreted within `systemId64`;
it is not assumed globally unique or automatically equivalent across sources.
ED-Finder's `bodies.id` is a local primary key, while journal/source identity is
retained separately as `system_id64` + `source_body_id` by ring ingestion.
Exploration projections may fall back to a lower-cased name
(`sql/044_exploration_projections.sql`). Names are labels, and these gaps are
not permission to force an unresolved match.
...
CRE owns researched mechanics, evidence, provenance, confidence, ontology and
Digital Twin state.
System Map owns spatial orientation and presentation. ED-Finder orchestrates and
presents; Babylon eventually renders. CPE owns plan construction, assessment,
recommendation, alternatives, sequencing and validation semantics. ED-Finder's
Colony Planner/Cockpit remains the current canonical detailed Build Plan
workspace and persistence owner. A future CPE plan identity, serialization, or
persistence boundary requires an explicit later design and migration. Spatial
contributions can carry a chosen plan, proposed facilities, alternatives,
rejected/blocked options and dependencies only when an owning plan-domain
