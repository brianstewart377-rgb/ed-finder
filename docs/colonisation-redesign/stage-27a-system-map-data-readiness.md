...
| Radius/mass/gravity/temp | corresponding `bodies` columns | ED-Finder | Nullable | Yes | UNKNOWN_COVERAGE | same | AUTHORITATIVE | `/api/system/{id64}` SELECT and `BodyModel` | Physical detail; separate semantic display scale | 27H/27I |
| Surface pressure | `bodies.surface_pressure` | ED-Finder | Nullable | **No** | UNKNOWN_COVERAGE | same | AUTHORITATIVE when projected with provenance | Stored/ingested, but absent from the current system-detail SELECT and `BodyModel` | Requires an explicit System Scene projection | 27H |
| Atmosphere type/composition | `bodies.atmosphere_type`, `atmosphere_composition` | ED-Finder | Nullable | **No** | UNKNOWN_COVERAGE | same | AUTHORITATIVE when projected with provenance | Stored/ingested, but absent from the current system-detail SELECT and `BodyModel` | Atmosphere visuals cannot rely on current System Detail | 27H/27I |
| Solid composition/materials/volcanism | `solid_composition`, `materials`, `volcanism` | ED-Finder | Nullable | **No** | UNKNOWN_COVERAGE | same | AUTHORITATIVE when projected with provenance | Stored/ingested, but absent from the current system-detail SELECT and `BodyModel` | Details/overlays need a new projection; never infer from class | 27H/27I |
| Landable/terraformable and world flags | `is_landable`, `is_terraformable`, Earth-like/water/ammonia booleans | ED-Finder | Yes, defaults false | Yes | UNKNOWN_COVERAGE | same | AUTHORITATIVE only when source completeness known | `/api/system/{id64}` SELECT and `BodyModel` | Default-false semantics need provenance audit; do not equate missing source with observed false. Richer stored `terraforming_state` is not exposed here | 27H |
...
| Stellar mass/luminosity/age/magnitude | `stellar_mass`, `luminosity`, `age_my`, `absolute_magnitude` | ED-Finder | Nullable | **No** | UNKNOWN_COVERAGE | same | AUTHORITATIVE when projected with provenance | Stored/ingested, but absent from the current system-detail SELECT and `BodyModel` | Requires an explicit System Scene projection | 27H/27I |
...
| Period/semi-major axis/eccentricity/inclination | `bodies.orbital_period`, `semi_major_axis`, `orbital_eccentricity`, `orbital_inclination` | ED-Finder | Nullable | **No** | UNKNOWN_COVERAGE | body update | AUTHORITATIVE when projected with provenance | Stored/ingested, but absent from the current system-detail SELECT and `BodyModel` | Orbit geometry needs an explicit projection and unit audit | 27H/27I |
| Ascending node/argument of periapsis/mean anomaly | corresponding `bodies` columns | ED-Finder | Nullable | **No** | UNKNOWN_COVERAGE | body update | AUTHORITATIVE when projected with provenance | Stored/ingested, but absent from the current system-detail SELECT and `BodyModel` | Still insufficient for present phase without units/epoch trust | 27H |
...
- CPE was re-audited read-only at
  `https://github.com/brianstewart377-rgb/colony-planning-engine`, revision
  `9d5ff8cc3cdd6653081f5f490dfd4b5b40423197`, path `README.md`. It is
  documentation-only: System Assessment and Colony Plan Construction are
  future capabilities, with no runtime, database, API, shared package, storage
  model, or implementation structure. ED-Finder currently persists local projects
...
Many rich fields are schema-capable and stored/ingested, but the current
`/api/system/{id64}` System Detail projection and `BodyModel` expose a much
narrower body/stellar/ring shape. Pydantic `extra='allow'` does not expose
columns the endpoint did not select. Schema breadth is promising, but production
population and units/provenance for
...
real System rendering must wait for 27H's explicit System Scene projection/API,
property-level provenance contract, and authorized production coverage audit.
