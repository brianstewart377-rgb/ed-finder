...
-- SELECT-only/read-only: no DML or DDL, no session-setting changes, no calls to
-- mutating or user-defined operational functions, and no repair/rebuild actions.
-- Ordinary PostgreSQL SELECT locking and built-in aggregates (COUNT/MIN/MAX)
-- still apply; this header does not claim a lock-free or function-free session.
