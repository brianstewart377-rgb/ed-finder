...
implementation, picking/projection and transitions. CRE owns researched
mechanics, evidence, provenance, confidence and Digital Twin reasoning. CPE
owns plan construction, assessment and recommendation semantics. ED-Finder
owns orchestration/presentation, and its Colony Planner/Cockpit remains the
current canonical detailed Build Plan workspace and persistence owner unless a
later explicit migration changes that boundary. Babylon renders.
...
`BodyRef` is renderer-neutral, not proof that ED-Finder's local `bodies.id` is
identical to a journal or other source BodyID. Every adapter must prove its
body-ID semantics within `systemId64`; names remain labels, and unresolved
cross-source joins remain unresolved. Ring ingestion's separate
`system_id64` + `source_body_id` discipline is the current safety precedent.
...
models directly or execute Preview. React/domain handlers decide whether an
explicit action is allowed.
