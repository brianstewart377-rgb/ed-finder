...
Stage 5F renders the Stage 5E comparison output in the optimiser details pane. The primary path compares the current editable Simulation Preview plan against the selected optimiser candidate, showing the conservative verdict, deterministic tradeoff summary, target-archetype change, facility and placement deltas, preview-summary deltas, ranking availability, warning/assumption changes, and risk direction. The comparison is advisory and preview-only: it does not run Simulation Preview, save a build, commit anything in-game, or alter backend mechanics.
...
| Candidate-vs-candidate | Engine support exists from Stage 5E; the compact UI path remains deferred to avoid clutter. |
| Safety | Comparison rendering is read-only and does not affect the Stage 5D load/confirmation flow. |
