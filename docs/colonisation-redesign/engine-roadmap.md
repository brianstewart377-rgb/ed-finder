...
| Boundaries | Stage 6C is comparison-only. It does not change predictions, alter optimiser candidate generation, alter optimiser ranking, change Simulation Preview scoring, mutate CP/economy/service/buildability mechanics, ingest EDMC or journals, or auto-correct predictions. A static passivity test asserts simulation/optimiser/ranking source files do not import `observations.comparison_engine` or `observations.store`. Stage 6D renders Stage 6C's output in the Simulation Preview UI. |
