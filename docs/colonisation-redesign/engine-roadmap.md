...
Stage 5.9E hardens the full Colony Planner workflow before Stage 6. Generated candidates remain visible when target archetype, maximum candidate count, or estimated-data settings become stale, but the UI shows generated/current values and cautions users before comparison or loading. Preview Result now marks an existing result stale when the editable Build Plan or target archetype changes after an explicit run; no automatic preview rerun is introduced.
