# EDRE ↔ ED-Finder Confidence Vocabulary Reconciliation
...
**Roadmap anchor:** `docs/ROADMAP.md` Foundation Sequence step 4 — *"EDRE
integration: confidence vocabulary first, then source authority, then release
artifact consumption"* — and its Do-Next line: *"Reconcile the EDRE and ed-finder
confidence vocabularies before consuming EDRE source-authority or release
...
begins**, and that EDRE's model **will become canonical**. This document is that
...
source-authority hierarchy. EDRE's `evidence/source_authority_register.md`
...
currently uses and EDRE's confidence/source-authority model; a statement of the
...
- **No code or schema change.** ED-Finder's `mechanics/confidence.py`,
  ...
  **not** modified by this document. The actual `confidence.py` redesign and any
  runtime translation layer are deferred until EDRE integration is separately
  authorized (roadmap Foundation Sequence step 4/5).
- **No runtime EDRE consumption.** EDRE is not wired into ED-Finder at runtime and
  nothing here wires it in.
- **No mechanics changes.** This reconciles *confidence vocabulary*, not
  colonisation mechanics truth. Mechanics truth remains EDRE's to define
  (`CLAUDE.md` three-repo boundary).
- **ED-Finder does not author EDRE's registers.** Where ED-Finder surfaces a
  concept EDRE's registers do not cover, it is recorded in §9 as *feedback to EDRE
  governance*, not resolved by editing EDRE from this repo.
- **The existing "confidence must not feed scoring" conservatism is retained.**
  ...
  ranking — consistent with EDRE CM-0006A (operational confidence ≠ research
...
incompatible scales**, none mapped to each other, let alone to EDRE:

| # | Vocabulary | Location | Scale / values | What it describes | EDRE layer (§6) |
|--:|---|---|---|---|---|
...
- **Axis conflation.** `ConfidenceLevel` mixes *who said it* (`verified`,
  ...
  (`inferred`, `estimated`, `speculative` — a **confidence-degree** claim). EDRE
  keeps these as separate concerns (SA-register vs CM-register).
- **No layer separation and no dynamics.** EDRE requires confidence to be tracked
...
This fragmentation is exactly the failure EDRE's model is built to prevent, which
is why EDRE's model is the right canonical target rather than a seventh
ED-Finder invention.

---

## 3. EDRE's canonical model (summary)
...
**Do not replace ED-Finder's vocabularies with a flat translation of EDRE's
score.** The correct move is to *locate each ED-Finder vocabulary in EDRE's layer
taxonomy first*, then map its values to (a) an EDRE **source-authority class** and
(b) an EDRE **confidence band**, keeping the two axes separate exactly as EDRE does.

The single most important realignment: **ED-Finder's `ConfidenceLevel` is not a
competitor to EDRE's evidence model — it is EDRE's *projected-operational* layer
...
ED-Finder surface its correct EDRE layer and a consistent band scale — not
...
EDRE's model canonical; ED-Finder's vocabularies become **derived projections**
of it, not independent sources of truth:

- Each confidence-bearing value in ED-Finder carries (or can be resolved to)
  three EDRE-aligned attributes: a **source-authority class** (SA-*), a **0–100
...
- EDRE's dynamics (decay, contradiction, negative evidence) become expressible;
  ED-Finder is not required to *compute* them until it consumes EDRE, but its
...
## 6. Layer map — where each ED-Finder surface belongs in EDRE

| ED-Finder surface | EDRE layer | Rationale |
|---|---|---|
...
| `ComparisonConfidence` / `…Impact` | **Evidence reconciliation** (CM-0002/CM-0005) | Precisely EDRE's "contradictions/negative evidence move trust" dynamic — but must stay advisory. |
...
Provisional mappings to be firmed up during integration. Bands are EDRE CM bands;
SA references are the dominant source class, not the only one.

### 7.1 `ConfidenceLevel` (projected-operational)

| ED-Finder | Confidence-degree reading | EDRE band (typical) | Source-authority note |
|---|---|---|---|
...
Already `0.0–1.0`. Map to EDRE bands by ×100: `≥0.85` High, `0.70–0.84` Usable,
`0.50–0.69` Exploratory, `<0.50` Weak. **Caveat:** these floats are *not* EDRE's
...
### 7.5 `ComparisonConfidenceImpact` → EDRE dynamics

| ED-Finder impact | EDRE dynamic |
|---|---|
...
This vocabulary already expresses EDRE's trust-movement dynamics — it is the
closest existing ED-Finder surface to EDRE's model — but per Stage 6C it remains a
...
When EDRE-to-ED-Finder integration is authorized, the translation layer must:

1. **Preserve both axes.** Never collapse source-authority (SA) into a single
   confidence degree, or vice-versa.
2. **Tag the layer.** Every translated value carries its EDRE layer; operational
   and evidence confidence are never silently equated (CM-0006A).
3. **Keep unknown unknown.** Absent EDRE data maps to "insufficient evidence,"
   never to a Weak-but-present band or to `0` (source-priority rule 5, SA-0010).
4. **One-way canonicity.** EDRE is the source of truth; ED-Finder projects. A
   round-trip must not let an ED-Finder display projection overwrite an EDRE score.
5. **No automation without band.** Only EDRE `High` (85–100) may drive automation;
   ED-Finder must not treat a projected-operational float ≥0.85 as if it were an
   evidence-High that clears EDRE's automation bar.
...
## 9. Gaps to raise with EDRE governance (feedback, not edits)

Recorded here as candidate feedback; **not** applied to EDRE from this repo:

- **Projected-operational sub-vocabulary.** ED-Finder's richest real-world need
  is operational confidence (is *this generated plan* reliable). EDRE names the
...
- **Analyst low/med/high shorthand.** ED-Finder and human reviewers use
  low/med/high heavily. Worth an explicit EDRE band-alias so the shorthand is
  governed rather than ad-hoc.
- **Catalogue `estimated` gate.** The binary "estimated vs known" catalogue gate
  is a useful, cheap SA-0002/CM-0007 signal; consider a named EDRE treatment.
...
- Any runtime EDRE consumption / translation-layer code.
