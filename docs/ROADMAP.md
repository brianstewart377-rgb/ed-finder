...
- **Map — spatial density pyramid (decoupled; #743 merged).** The density pyramid
  ...
  (`v3-spatial-pyramid.yml`, with `build`|`publish` sub-commands) remains
  **fail-closed-blocked until `ratings_v4_prod_p4_opt1` reaches READY**; publish is
  a separate CAS step (`publish_spatial_pyramid`), then an application deploy — a
  deliberate, deferred owner step. Note: per the feature-PR precedent (F1 `010`,
...
- **Ops — production version-drift monitor (#741, merged).** An automated monitor
  compares the deployed build (`/_app/version.json` and `/api/health` `build_sha`)
  against the expected release and surfaces drift, so an un-redeployed artifact is
  detected rather than mistaken for a fresh deploy.
- **Journal import — edge batch limit (#742, merged).** Verified-journal import
  now keeps request batches under the ~1 MB edge limit, complementing the
