...
- Map layer posture (2026-08-08): the map's typed `MapSceneDescriptor` layer/adapter
  ...
  August 8 layer/adapter pattern is retained as Stage 26 history and value.
  Stage 27 supersedes only its global Explore-only restriction while preserving
  explicit ownership and truth boundaries.
...
CPE owns plan-domain construction, assessment, and recommendation semantics;
ED-Finder's Colony Planner/Cockpit remains the current canonical detailed Build
Plan workspace and persistence owner unless a later explicit migration changes
that boundary. ED-Finder orchestrates/presents, and the renderer owns neither
domain semantics nor persistence.
