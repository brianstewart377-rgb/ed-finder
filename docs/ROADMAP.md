...
### Current audit-response follow-through

Stage 27A and the Stage 27 programme above govern spatial work. The items in
this subsection are non-spatial foundation follow-through unless explicitly
...
- **Stage 26-era reusable evidence, not current sequencing:** the bounded
  `/api/map/systems` viewport work, spatial-index findings, importance ranking,
  response cap, client LOD experiments, hysteresis and truncation affordance in
  `docs/superpowers/plans/2026-08-11-map-real-star-streaming.md` remain useful
  inputs. Its former Explore-only Phase 1/2/3 programme is superseded; Stage
  27D owns real-system streaming and interaction, and Stage 27E owns inherited
  Stage 26 capability parity. Neither stage is authorized by 27A.
...
1. Complete and accept Stage 27A's spatial product, architecture,
   data-readiness, migration and governance contracts. Its only follow-on
   authorization is isolated Stage 27B; do not begin 27B in 27A.
2. Keep the deployed Stage 26E R3F map stable as the current production
   baseline, maintenance reference, inheritance source and rollback while a
   later Stage 27 bakeoff/cutover remains unearned.
3. Preserve production data-integrity receipts and the bounded rerating cadence.
4. Complete dependency-aware documentation triage and historical archiving.
5. Finish the archetype-scoring pivot and retire legacy score storage safely.
6. Reconcile CRE confidence/source-authority contracts before runtime integration.
7. Maintain all ten protected CI checks, reproducible release artifacts, local
   parity, and the green isolated Review Lab browser workflow.
8. Preserve the reviewed database-operator secret channels, finite migration
   timeout policy, and explicit exceptional-run opt-in.
9. Continue planner trust, evidence clarity, and operator reviewability.
10. Keep product-shell and selected-system continuity stable while foundations
   evolve.
...
- Spatial-platform work is authorized only through the current Stage 27
  sequence and contracts. In that sequence it may participate explicitly
  across Explore -> Inspect -> Plan -> Review. Colony Planner/Cockpit remains
  the canonical detailed Build Plan workspace and persistence owner; the
  renderer owns no mechanics or planning logic.
- Spatial handoffs and explicit map actions authorized by a Stage 27 slice are
  allowed. No map action may silently mutate a Build Plan or execute Preview,
  and planned, inferred, or schematic state must never masquerade as existing
  fact. Detailed plan persistence, mutation and automation remain outside the
  renderer and may be implemented only in the authorized later stage that owns
  them.
...
- [`colonisation-redesign/spatial-platform-product-contract.md`](./colonisation-redesign/spatial-platform-product-contract.md):
  current Stage 27 spatial product, truth, workflow and Commander
  History/Journal authority once Stage 27A is accepted.
- [`colonisation-redesign/spatial-platform-architecture-decision.md`](./colonisation-redesign/spatial-platform-architecture-decision.md):
  current Stage 27 renderer-neutral ownership and architecture authority once
  Stage 27A is accepted.
- [`colonisation-redesign/stage-26a-next-generation-map-foundation-contract.md`](./colonisation-redesign/stage-26a-next-generation-map-foundation-contract.md):
  historical Stage 26 authorization and requirements evidence.
- [`colonisation-redesign/stage-26b-renderer-bakeoff-decision.md`](./colonisation-redesign/stage-26b-renderer-bakeoff-decision.md),
  [`colonisation-redesign/stage-26c-region-first-foundation-contract.md`](./colonisation-redesign/stage-26c-region-first-foundation-contract.md),
  [`colonisation-redesign/stage-26d-feature-handoff-contract.md`](./colonisation-redesign/stage-26d-feature-handoff-contract.md), and
  [`colonisation-redesign/stage-26e-cutover-readiness.md`](./colonisation-redesign/stage-26e-cutover-readiness.md):
  historical Stage 26 renderer decision, implementation contracts and stable
  production-baseline/cutover evidence. Babylon did not win Stage 26.
...
- [`colonisation-redesign/stage-25c-product-shell-shared-context-contract.md`](./colonisation-redesign/stage-25c-product-shell-shared-context-contract.md):
  settled historical product-shell and shared-context baseline.
- [`colonisation-redesign/stage-25b-evidence-language-visual-primitives.md`](./colonisation-redesign/stage-25b-evidence-language-visual-primitives.md):
  settled historical evidence-language and visual-system baseline.
- [`colonisation-redesign/stage-25a-current-state-map-product-visual-baseline.md`](./colonisation-redesign/stage-25a-current-state-map-product-visual-baseline.md):
  historical current-state audit and map product baseline from Stage 25.
