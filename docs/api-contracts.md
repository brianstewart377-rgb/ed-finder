...
ED-Finder keeps the backend, the `apps/web` frontend, and the EDDN worker as separate apps. They should stay separate. The shared contract between them is the HTTP API.

> **Historical note.** The current frontend is `apps/web` (Svelte, pnpm). Sections below that describe pre-V3 Stage work still reference the retired React `frontend/` tree and its `frontend/src/lib/api/` helpers — those are historical evidence of what shipped at the time, not current instructions. New work uses the `apps/web` facade and the `pnpm generate:api` flow above.
...
Local schema generation and the drift launcher require the repository's exact
CPython 3.14.x execution surface; interpreter overrides are accepted only when
they identify themselves as CPython 3.14.x.

The Svelte client under `apps/web/src/lib/api/generated` is generated by Hey API from the same running FastAPI document. Its generator requires an explicit `OPENAPI_INPUT` URL or path and has no checked-in schema fallback:
...
On PowerShell (the Windows-first shell):

```powershell
cd apps/web
$env:OPENAPI_INPUT = "http://127.0.0.1:8000/openapi.json"; pnpm generate:api
```

## Frontend Types

The current web app lives in `apps/web` (Svelte 5 / SvelteKit 2). Its typed
client under `apps/web/src/lib/api/generated` is generated by Hey API from the
running FastAPI OpenAPI document — use the `pnpm generate:api` flow shown above
(bash or PowerShell).

If the API is not already running locally, start the backend first (see the
repository README and `docker-compose.review.yml` / local dev compose), then
regenerate. The retired React `frontend/` tree and its `yarn types:gen` flow are
historical evidence only and must not be used.

## Frontend API Calls

Frontend calls must go through the generated API facade in `apps/web/src/lib/api/`
(the API barrel), rather than importing generated modules directly or
reconstructing request shapes in components.

Use the generated facade functions exported from `apps/web/src/lib/api/` rather
than the retired `frontend/` planner helpers (`getSlotPredictions`,
`getBuildability`, `getSimulationSummary`), which are historical and are not part
of the V3 app.
...
3. Regenerate the OpenAPI types with `pnpm generate:api` (from `apps/web`).
4. Update the relevant module under `apps/web/src/lib/api/` if a new endpoint or helper is needed (or add a new domain file and wire it into `index.ts` if none fits).
5. Update `apps/web` components to use the generated types and the central client facade.
