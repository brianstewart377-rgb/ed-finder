...
work prepares tooling; production commands and private receipts run only in the
Hetzner operator shell. The readiness audit is read-only and is evidence, not
...
The safe profile uses catalog estimates for large base tables and skips costly
coverage joins. Schedule `--full` separately when exact scans of the roughly
188M-system database are acceptable. A skipped, timed-out, or errored required
check is never equivalent to zero or ready.
...
2. **Schema/migrations:** PostgreSQL major version is 18; manifest and ledger
...
11. **Materialized views/stats/cache:** every schema-supported map and archetype MV
    is present, populated/readable, and has required unique indexes; refresh/stats
