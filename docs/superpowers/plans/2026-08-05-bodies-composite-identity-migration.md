...
- **The stated next action is NOT simply "follow the rollout runbook" — Codex Review caught a real gap in the runbook itself.** `scripts/deploy_main.sh:131` calls `bash scripts/apply_migrations.sh` with no `--include-manual` flag, and `scripts/apply_migrations.sh:245-246` explicitly *skips* any migration in `manual` mode (which 041 is) unless `--include-manual` is passed. The rollout runbook's own Step 1 ("Deploy Task 1 alone... via `scripts/deploy_main.sh`") would silently skip this migration entirely and report success — this is a bug in the original 2026-08-05 plan, not something introduced tonight. The actual required invocation is a direct, explicit run:
  ```
  bash scripts/apply_migrations.sh --include-manual
  ```
  with `MIGRATION_STATEMENT_TIMEOUT` (default `1h`) very likely needing a generous override — a `CREATE INDEX CONCURRENTLY` on ~573M rows can plausibly run for multiple hours per this plan's own text, and the default 1h timeout would abort it mid-build, leaving an *invalid* index that then has to be dropped and retried. Whoever runs this should decide a concrete timeout value (or use `EDFINDER_ALLOW_UNBOUNDED_MIGRATION_TIMEOUTS=yes` with 0, monitoring manually via `pg_stat_progress_create_index` as the rollout runbook already describes) rather than trust the 1h default. This env var/flag detail was not yet resolved as of this status check — decide it before running, don't guess at execution time.
- **Tasks 2-6: not started.** `sql/001_schema.sql` still shows bare `id BIGINT PRIMARY KEY` on `bodies`. `apps/eddn/src/eddn_listener.py` still writes `ON CONFLICT (id)`. `apps/importer/src/import_spansh.py` still calls `upsert_via_temp(conn, 'bodies', BODY_COLS, body_batch, 'id', ...)` at its bodies call site (line 953-954, spans a line break — see the corrected Step 3 note below) — bare string conflict target, not yet `['id', 'system_id64']`.
...
- **This status check itself is only a partial Task 0 re-verification.** Steps 1 and 3 were re-run and confirmed (see their checkboxes below). Steps 2 (dependent tables still carry `system_id64`) and 4 (no new unscoped `bodies` read site) were *not* re-checked tonight — Codex Review correctly flagged an earlier draft of this status section for implying full re-verification when it wasn't. Run Steps 2 and 4 before treating this plan's blast-radius assumptions as current, not just Steps 1 and 3.
...
- [x] **Step 3: Confirm the two writers are still the only writers of `bodies`** — re-verified 2026-08-07: `eddn_listener.py` still does `ON CONFLICT (id)` (composite conflict target not yet applied — matches "Tasks 2-6 not started" above), `import_spansh.py` still calls `upsert_via_temp(conn, 'bodies', BODY_COLS, body_batch, 'id', ...)` at its bodies call site (lines 953-954: `_count, rejected_owner_keys = upsert_via_temp(\n    conn, 'bodies', ...)`, spanning a line break) with the bare-string conflict_col form, no third writer found. (Codex Review caught that the second grep below silently finds nothing for the importer: the exact string `upsert_via_temp(conn, 'bodies'` never occurs on one line since the call wraps, and `-A2` — trailing-context-after-a-match — can't fix that when there's no matching line to begin with. Use `grep -n "upsert_via_temp" apps/importer/src/import_spansh.py` and inspect the call sites directly, or `rg -U` for a true multiline match, instead of the single-line pattern below.)
